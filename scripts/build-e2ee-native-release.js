...
function buildNativeRelease({ root, privateKeyFile, outputDirectory, platform = process.platform, arch = process.arch }) {
  if (!privateKeyFile || !fs.statSync(privateKeyFile).isFile()) throw new Error('E2EE_RELEASE_PRIVATE_KEY_REQUIRED');
  ...
  const publicKey = crypto.createPublicKey(fs.readFileSync(privateKeyFile));
  ...
  const signature = crypto.sign(null,Buffer.from(JSON.stringify(unsigned)),fs.readFileSync(privateKeyFile)).toString('base64');
