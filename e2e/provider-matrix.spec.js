...
test('delivery_modes order controls Pull versus Push and recovers without bypassing order', async ({ request }, testInfo) => {
  ...
  await setModes(request, 'e2e-agent', ['pull', 'mock-echo']);
  await expect.poll(async () => (await runtime(request, 'e2e-agent')).deliveryStatus.activeMode).toBe('pull');
  ...
  await setModes(request, 'e2e-agent', ['mock-echo', 'pull']);
  await expect.poll(async () => (await runtime(request, 'e2e-agent')).deliveryStatus.activeMode).toBe('mock-echo');
  await inject(request, {
  ...
  await expect.poll(async () => (await runtime(request, 'e2e-agent')).deliveryStatus.activeMode).toBe(null);
});
...
test('provider availability is isolated per Agent and restores the affected route only', async ({ request }, testInfo) => {
  ...
  await expect.poll(async () => (await runtime(request, 'e2e-agent')).deliveryStatus.activeMode).toBe('mock-echo');
  await expect.poll(async () => (await runtime(request, 'e2e-agent-2')).deliveryStatus.activeMode).toBe('pull');
  ...
  await expect.poll(async () => (await runtime(request, 'e2e-agent-2')).deliveryStatus.activeMode).toBe('mock-echo');
  await inject(request, {
    toUid: 'e2e-im-uid-2', fromUid: 'e2e-visitor', channelId: secondChannel, channelType: 1,
