...
Codex on Windows does not currently pass its thread ID to a stdio MCP child. VOKO provides an opt-in identity handshake for this case. Start the VOKO runtime with `VOKO_MCP_IDENTITY_HANDSHAKE=1`, then let Codex follow the structured `nextAction` returned by `voko_whoami`: it calls `voko_prepare_identity_handshake`, runs the returned command once in its current shell, and retries `voko_whoami`. The command contains a short-lived one-time challenge; Codex must not display, edit, persist, or reuse it. If the feature is disabled or the handshake fails, use explicit Agent selection instead.

The handshake binding is held only in the running VOKO process. Restarting VOKO or replacing the MCP connection requires a new handshake. It does not infer identity from process recency and does not expose the native Provider session ID to MCP output.

The handshake response includes a timing estimate. VOKO's challenge, validation, binding, and database lookup are normally sub-second; the verified Windows Codex end-to-end flow took about 80 seconds because it spans multiple model and shell-tool turns. Linux normally avoids this bridge: VOKO can resolve one unique Codex rollout session through the caller process chain under `/proc`. Restricted `/proc`, containers, or multiple simultaneous rollout descriptors can prevent that automatic detection; in those exceptional cases VOKO still refuses to guess and requires the handshake or explicit Agent selection.
