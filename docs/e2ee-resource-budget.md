...
`npm run test:e2ee:stability` runs a 30-minute release-mode bidirectional MLS loop by default. It periodically rejects replayed ciphertext, serializes and restores both endpoint states, performs PCS self-update Commits and emits `e2ee-stability-summary.json` with message, recovery, update, loss, duplicate-delivery, crossed-session, RSS, P95 operation latency and throughput counters. Latency uses a fixed-size histogram and RSS is sampled instead of retaining per-message measurements, so the monitor remains bounded during 4-12 hour runs. A shorter diagnostic run can use `npm run test:e2ee:stability -- --duration=5s`; only the full duration is a release gate.

The first bounded-metrics Windows diagnostic on 2026-08-16 processed 2,017 bidirectional messages in 5.002 seconds, including 20 rejected replays, eight state restores and four PCS updates. It reported 7,995,392 bytes starting RSS, 8,646,656 bytes peak RSS, 237 microseconds operation P95 and 403.2 messages/second, with zero loss, duplicate delivery or crossed sessions. This short run validates the sampler and remains non-substitutive for the 30-minute gate.
