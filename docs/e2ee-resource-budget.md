...
`npm run test:e2ee:stability` runs a 30-minute release-mode bidirectional MLS loop by default. It periodically rejects replayed ciphertext, serializes and restores both endpoint states, performs PCS self-update Commits and emits `e2ee-stability-summary.json` with message, recovery, update, loss, duplicate-delivery and crossed-session counters. A shorter diagnostic run can use `npm run test:e2ee:stability -- --duration=5s`; only the full duration is a release gate.
