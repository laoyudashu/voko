...
def run(host):
 ...
 d=json.loads(h.run(host,[h.NODE[host],'-e',expr],timeout=75,input=archive));d['archiveSha256']=hashlib.sha256(archive).hexdigest();(r/(host+'-final-v2-runtime.json')).write_text(json.dumps(d,indent=2))
 try:
  out=h.run(host,[h.NODE[host],'--test',d['root']+'/test/provider-native-policy-matrix.test.js'],timeout=150);(r/(host+'-final-v2-regression.log')).write_text(out);d['status']='passed'
 except Exception as e:(r/(host+'-final-v2-regression.log')).write_text(str(e));d['status']='failed'
 (r/(host+'-final-v2-runtime.json')).write_text(json.dumps(d,indent=2));print(json.dumps({'host':host,'status':d['status']}),flush=True)
