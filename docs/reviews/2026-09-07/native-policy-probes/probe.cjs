...
const mode=process.argv[6]||'default';
...
cp.spawn=function(...args){const argv=args[1]||[];const env=args[2]?.env||{};launches.push({flags:argv.filter(x=>typeof x==='string'&&/^--(?:no-|safe|pure|bare|permission|approval|tools|exclude|deny|strict|sandbox|max|disable|plan|trust)/.test(x)),env:Object.fromEntries(['QWEN_CODE_SAFE_MODE','CLINE_COMMAND_PERMISSIONS','GEMINI_SANDBOX','OPENCODE_CONFIG_CONTENT','OPENCODE_DISABLE_PROJECT_CONFIG'].map(k=>[k,env[k]===undefined?'absent':k==='OPENCODE_CONFIG_CONTENT'?'present':env[k]]))});const child=originalSpawn.apply(this,args);activeChildren.add(child);child.once('close',()=>activeChildren.delete(child));return child;};
...
async function finish(reason){if(finished)return;finished=true;clearTimeout(timer);result=result||{};result.observations=check();result.acpPermissionRequestsDenied=(diagnostic.match(/ACP tool permission denied by default/g)||[]).length;result.elapsedMs=Date.now()-started;result.mode=mode;result.launches=launches;result.status=Object.entries(result.observations).some(([k,v])=>k!=='controlReplyObserved'&&v)?'CAPABILITY_OBSERVED':reason|| (result.observations.controlReplyObserved?'NO_CANARY_EFFECT_OBSERVED':'INCONCLUSIVE_NO_CONTROL');
...
const started=Date.now();const timer=setTimeout(()=>finish('TIMEOUT_UNVERIFIED'),65000);
(async()=>{
 ...
 result={at:new Date().toISOString(),transportId,family:def.family,platform:process.platform,release:os.release(),arch:os.arch(),node:process.version,scope:'fresh synthetic Agent and conversation; resolved VOKO visitor policy; requested '+mode,testProfileBindingPresent:!!instanceBinding,workingDirectory:provider._cwd||null,harnessSha256:crypto.createHash('sha256').update(fs.readFileSync(__filename)).digest('hex'),runtime:runtime?{executable:runtime.executable,argvPrefix:runtime.argvPrefix,fingerprint:runtime.fingerprint,runtimeKind:runtime.runtimeKind}:null};
 ...
 const {ProviderSecurityPolicyService,appendProviderSecurityPrompt}=require(path.join(root,'build/core/provider-security-policy'));
 const {snapshotFromProvider}=require(path.join(root,'build/core/provider-capability'));
 const security=new ProviderSecurityPolicyService(db);
 const original=security.effective(agentId,transportId).config;
 const config={...original};
 if(mode==='native'){
  const choices={executionMode:'native',toolProfile:'default',safeMode:'disabled',toolAccess:'default',customizations:'default',sandboxMode:'native',extensionProfile:'default',pluginMode:'default',mcpProfile:'user',dataFileAccess:'read_write'};
  for(const [k,v] of Object.entries(choices))if(k in config)config[k]=v;
  if(transportId==='claude-cli'||transportId==='workbuddy-http')config.permissionMode='default';
  if(transportId==='qwen-office-cli')config.permissionMode='bypass_permissions';
  const preflight=security.preflight(agentId,transportId,config);security.commit(agentId,preflight.preflightToken,agentId);
 }
 result.requestedConfig=Object.fromEntries(Object.entries(config).filter(([k])=>k!=='additionalPrompt'));
 ...
 security.storeCapability(agentId,transportId,snapshotFromProvider(provider,transportId,agentId));
 ...
 if('_timeout' in provider)provider._timeout=55000;
 ...
 const lease=security.acquireTurnLease({agentId,fromUid:'synthetic-visitor',channelType:1,sourceType:'visitor',messageId:control,turnId:control},transportId);
 result.securityPolicy=lease?{transportId:lease.transportId,config:lease.config,policyDigest:lease.policyDigest,capabilityDigest:lease.capabilityDigest,nativePolicyState:lease.nativePolicyState}:null;
 if(result.securityPolicy?.config)result.securityPolicy.config=Object.fromEntries(Object.entries(result.securityPolicy.config).filter(([k])=>k!=='additionalPrompt'));
 await provider.start();
 try{await provider.push({providerSecurityPolicy:lease,agentId,fromUid:'synthetic-visitor',senderUid:'synthetic-visitor',channelId:'synthetic-visitor',channelType:1,content:appendProviderSecurityPrompt(prompt,lease),rawContent:prompt,messageId:control,turnId:control,timestamp:Date.now(),sourceType:'visitor'});}
