...
(async()=>{
 ...
 if(referenceDbPath){const {DatabaseSync}=require('node:sqlite');const reference=new DatabaseSync(referenceDbPath,{readOnly:true});try{const row=reference.prepare("SELECT backend_instance_id FROM agents WHERE backend_type=? AND agent_name LIKE 'TEST-%' AND backend_instance_id IS NOT NULL AND backend_instance_id!='' ORDER BY agent_name LIMIT 1").get(def.family);if(row){instanceBinding=row.backend_instance_id;db.prepare('UPDATE agents SET backend_instance_id=? WHERE agent_id=?').run(instanceBinding,agentId);}}finally{reference.close();}}
 ...
 const runtime=typeof provider._resolveRuntime==='function'?provider._resolveRuntime():null;
 result={at:new Date().toISOString(),transportId,family:def.family,platform:process.platform,release:os.release(),arch:os.arch(),node:process.version,scope:'fresh synthetic Agent and conversation; adapter defaults',testProfileBindingPresent:!!instanceBinding,runtime:runtime?{executable:runtime.executable,argvPrefix:runtime.argvPrefix,fingerprint:runtime.fingerprint,runtimeKind:runtime.runtimeKind}:null};
 if(!provider.isAvailable(agentId))return finish('RUNTIME_UNAVAILABLE');
 ...
 provider.on('agent.reply',r=>{reply+=String(r.content||'');});
