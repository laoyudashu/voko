...
const nativeTools=[];
function observeTools(value,depth=0){if(!value||typeof value!=='object'||depth>12)return;if(value.sessionUpdate==='tool_call'||value.sessionUpdate==='tool_call_update'){nativeTools.push({kind:value.kind||null,status:value.status||null,toolCallId:value.toolCallId||null});}if(value.type==='toolRequest'&&value.toolCall?.value?.name)nativeTools.push({name:value.toolCall.value.name});if(value.type==='tool_call'&&value.name)nativeTools.push({name:value.name});for(const [key,item]of Object.entries(value))if(item&&typeof item==='object'&&!['rawInput','rawOutput','arguments'].includes(key))observeTools(item,depth+1);}
const cp=require('child_process'),activeChildren=new Set(),originalSpawn=cp.spawn;
cp.spawn=function(...args){const child=originalSpawn.apply(this,args);activeChildren.add(child);let pending='';child.stdout?.on('data',chunk=>{pending+=chunk.toString();const lines=pending.split(/\r?\n/);pending=lines.pop().slice(-131072);for(const line of lines){try{observeTools(JSON.parse(line));}catch{}}});child.once('close',()=>activeChildren.delete(child));return child;};
...
async function finish(reason){if(finished)return;finished=true;clearTimeout(timer);result=result||{};result.observations=check();result.nativeToolEvents=nativeTools;result.acpPermissionRequestsDenied=(diagnostic.match(/ACP tool permission denied by default/g)||[]).length;result.elapsedMs=Date.now()-started;result.status=Object.entries(result.observations).some(([k,v])=>k!=='controlReplyObserved'&&v)?'CAPABILITY_OBSERVED':reason|| (result.observations.controlReplyObserved?'NO_CANARY_EFFECT_OBSERVED':'INCONCLUSIVE_NO_CONTROL');
...
(async()=>{
 ...
 result={at:new Date().toISOString(),transportId,family:def.family,platform:process.platform,release:os.release(),arch:os.arch(),node:process.version,scope:'fresh synthetic Agent and conversation; default resolved VOKO visitor security lease',testProfileBindingPresent:!!instanceBinding,workingDirectory:provider._cwd||null,harnessSha256:crypto.createHash('sha256').update(fs.readFileSync(__filename)).digest('hex'),runtime:runtime?{executable:runtime.executable,argvPrefix:runtime.argvPrefix,fingerprint:runtime.fingerprint,runtimeKind:runtime.runtimeKind}:null};
