...
The cross-process canary separates those endpoints: Chromium/WASM owns only the creator state, a distinct Lite process owns the recipient KeyPackage private material and joined MLS state, and a Node relay stores only Commit, Welcome, acknowledgement and application ciphertext. The creator waits for and decrypts the MLS-authenticated establishment acknowledgement before sending the first application message. This is still a test canary, not production message-path enablement.
