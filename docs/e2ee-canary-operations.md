...
> 历史工程资料：本手册只适用于与正式运行时互斥的内部 Canary、回归和故障演练。普通用户启用方式见[访客私聊端到端加密](e2ee-private-chat.md)。Canary 结果不能替代当前生产版本验收，也不能表述为独立外部审计。

本手册只适用于精确白名单中的测试主人、Agent 和设备。Canary 不得与正式生产 E2EE 同时开启，也不得向普通用户展示。
