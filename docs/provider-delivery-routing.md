...
- 原生 session 恢复失败时不会猜测最近会话。只有能证明当前请求尚未提交，才允许相应的绑定恢复或安全重投；不能用 CLI 输出中的登录、权限或 session 错误文字作为未执行的证明。
- CLI 已启动后异常退出或返回无法确认执行阶段的错误时，结果为 `outcome_unknown`；仅真实启动前失败可归类为 `not_delivered`。投递结果不明确时不跨通道自动重发；消息保留在 VOKO，优先通过 Pull 确认，避免重复回复。
