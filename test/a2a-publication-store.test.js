...
test('A2A publication requires an explicit declaration and persists the choice independently', t => {
  ...
  assert.equal(store.getPublicEnabled('agent-1'), null); assert.equal(store.isPublicEnabled('agent-1'), false); store.setPublicEnabled('agent-1', false);
