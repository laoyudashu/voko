...
function scenario(t,execute){const db=fixture(t);let queued=true,finish=[],uploads=0,options;
 ...
 if(action==='assetPrepare'){uploads++;data={id:'asset',url:'https://cloud.invalid/test',headers:{}};}
 ...
 const worker=createProjectWorker({db,client,dispatcher,fetchImpl:async()=>({ok:true})});
