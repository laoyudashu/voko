...
function renderExpandableMentionContent(content,mention,members,tFn){
  const raw=String(content||'');
  if(raw.length<=500)return renderMentionContent(raw,mention,members,tFn);
  const shortHtml=renderMentionContent(raw.substring(0,500)+'…',mention,members,tFn);
  const fullHtml=renderMentionContent(raw,mention,members,tFn);
  return '<span data-voko-expandable><span data-voko-message-preview>'+shortHtml+'</span><span data-voko-message-full hidden>'+fullHtml+'</span><button type="button" data-voko-expand-message aria-expanded="false" style="display:block;margin:5px 0 0;padding:2px 8px;min-width:auto;min-height:auto;font-size:13px;line-height:1.4">'+esc(tFn('web.conversation.expand'))+'</button></span>';
}
    ...
    try{
      ...
      if(msgs.length){mh='';for(const m of msgs){
        if(m.contentType===11){
          ...
          const contentHtml=mediaHtml||renderExpandableMentionContent(rawContent,m.mention,ctx.members,T);
      ...
      const quitDlg='<dialog id="quit-dlg" class="gm-dialog"><div class="gm-dialog-body"><div class="gm-dialog-icon" aria-hidden="true">!</div><h3>'+L('web.group.btn.quit')+'</h3><p>'+quitConfirm+'</p></div><div class="gm-dialog-actions"><button type="button" class="btn btn-outline" onclick="closeQuitDlg()">'+L('common.btn.cancel')+'</button><button type="button" class="btn btn-danger" onclick="confirmQuitGroup()">'+L('common.btn.confirm')+'</button></div></dialog>';
...
function _gmExpandableHtml(content,mention){var raw=String(content||"");if(raw.length<=500)return _gmMentionHtml(raw,mention);var labels=window.__vokoExpandLabels||{expand:"展开全文",collapse:"收起全文"};return '<span data-voko-expandable><span data-voko-message-preview>'+_gmMentionHtml(raw.slice(0,500)+"\\u2026",mention)+'</span><span data-voko-message-full hidden>'+_gmMentionHtml(raw,mention)+'</span><button type="button" data-voko-expand-message aria-expanded="false" style="display:block;margin:5px 0 0;padding:2px 8px;min-width:auto;min-height:auto;font-size:13px;line-height:1.4">'+_gmEsc(labels.expand)+'</button></span>';}
...
window.__gmAdd=function(m){var bx=document.getElementById("msg-box");if(!bx)return;var sender=m.senderName||_gmNameOf(m.fromUid);var h,tt=new Date((m.timestamp||0)*1000).toLocaleTimeString();if(m.contentType===11){h=_gmAuditHtml(m.content,tt);}else if(m.contentType===12){h='<div class="tip">'+_gmEsc(_gmRenderTip(m.content))+' <span style="font-size:12px;color:#aaa">'+tt+'</span></div>';}else{var tm=tt,media=window.__vokoMessageRenderer.renderMedia(m.contentType,m.content),ct=String(m.content||"");var atMe=_gmMentioned(m),badge=atMe?' <span class="gm-at-me-badge">'+_gmEsc(_GM_ME)+"</span>":"";h='<div class="gm-message'+(atMe?" gm-message-at-me":"")+'">'+_gmSenderHtml(m.fromUid,sender)+badge+' <span style="color:#888;font-size:13px">['+tm+"]</span><br>"+(media||_gmExpandableHtml(ct,m.mention))+"</div>";}bx.insertAdjacentHTML("beforeend",h);bx.scrollTop=bx.scrollHeight;};
