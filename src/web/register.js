...
function addAgentWizardBody(email, categories, db, tFn) {
  ...
  return '<div class="voko-logo">VOKO</div>'
    ...
    + '<section class="wizard-panel" data-step="4"><h3 id="wf-step4-title">' + esc(t('register.flow.done.title')) + '</h3><div id="wf-result"></div></section>'
    + '</div>'
...
function wizardJs(t, manualCommands = {}) {
  const I = {
    ...
    instanceOptional: t('register.flow.provider.instance_optional'),
    instanceRequiredNone: t('register.flow.provider.instance_required_none'), instanceRequiredSelect: t('register.flow.provider.instance_required_select'),
    nameTaken: t('register.add.name_taken'),
    nameCheckUnavailable: t('register.add.name_check_unavailable'),
    searchableBy: t('register.flow.done.searchable_by'),
...
(function(){
  ...
  if(!root)return;
  var step=1, regId='', state=null, selectedProvider='', selectedInstance='', configMode='', discardDraft=false, detectionPromise=null, workbuddyLoad='idle', workbuddyError='';
  ...
  function show(n){step=n;panels.forEach(function(p,i){p.classList.toggle('active',i===n-1)});steps.forEach(function(s,i){s.classList.toggle('active',i===n-1);s.classList.toggle('done',i<n-1)});prev.style.visibility=n===1||state&&state.status==='created'?'hidden':'visible';next.textContent=n===4?I.enter:n===3?I.create:I.next;saveDraft()}
  ...
  function saveDraft(){if(discardDraft)return;try{sessionStorage.setItem(draftKey,JSON.stringify({email:root.dataset.email,registrationId:regId,step:step,name:nameInput.value,description:document.getElementById('wf-desc').value,category:document.getElementById('wf-category').value,provider:selectedProvider,instance:selectedInstance}))}catch(_){}}
  function applyDraftFields(d){if(!d)return;nameInput.value=d.name||nameInput.value;document.getElementById('wf-desc').value=d.description||'';if(d.category)document.getElementById('wf-category').value=d.category;selectedProvider=d.provider||'';selectedInstance=d.instance||''}
  function restore(d){
    state=d;
    ...
    else if(d.status==='ready_to_create'){if(d.environment)renderProviders(d.environment);renderDeliveries(d);show(3)}
  ...
  function completeRegistration(){next.textContent=I.creating;api('complete',{}).then(async function(d){if(selectedIconFile){next.textContent=I.iconUploading;var uploaded=await uploadSelectedIcon(d.result.agentId);d.result.iconUrl=uploaded.iconUrl}state=d;renderResult(d);show(4);next.disabled=false}).catch(fail)}
  next.onclick=function(){
    next.disabled=true;
    if(step===1){if(!selectedProvider){next.disabled=true;return}var chosen=(state.environment.detected||[]).find(function(p){return p.type===selectedProvider});if(chosen&&chosen.requiresInstance&&!selectedInstance&&(chosen.instances||[]).length!==1){fail(new Error(I.instanceRequiredSelect.replace('{provider}',chosen.label).replace('{instance}',chosen.instanceTerm||I.instance)));return}api('select_provider',{providerType:selectedProvider,instanceId:selectedInstance}).then(function(d){state=d;applySuggestion(d.suggestedBasicInfo);show(2);next.disabled=false}).catch(fail);return}
    ...
    if(step===3){api('select_delivery',{deliveryModes:selectedModes()}).then(function(d){state=d;completeRegistration()}).catch(fail);return}
  ...
  function renderResult(d){var r=d.result||{},p=r.provider||{},rows=(r.deliveryOrder||[]).map(function(m){var role=m.role==='primary'?I.priority:m.role==='fallback'?I.backup:m.role==='only'?I.only:I.finalFallback;return '<div>'+m.priority+'. '+escHtml(m.label)+' <span class="tag">'+escHtml(role)+'</span></div>'}).join('');document.getElementById('wf-step4-title').textContent=I.created;document.getElementById('wf-result').innerHTML='<div class="result-card"><h3>✓ '+escHtml(I.created)+'</h3><dl class="result-grid"><dt>'+escHtml(I.name)+'</dt><dd>'+escHtml(r.agentName)+'</dd><dt>'+escHtml(I.description)+'</dt><dd>'+escHtml(r.description||'-')+'</dd><dt>'+escHtml(I.category)+'</dt><dd>'+escHtml(r.category)+'</dd><dt>'+escHtml(I.provider)+'</dt><dd>'+escHtml(p.type||'others')+'</dd><dt>'+escHtml(I.instanceLabel)+'</dt><dd>'+escHtml(p.instanceName||'-')+'</dd><dt>'+escHtml(I.deliveryOrder)+'</dt><dd>'+rows+'</dd></dl><h4>'+escHtml(I.searchableBy)+'</h4><dl class="result-grid"><dt>'+escHtml(I.searchName)+'</dt><dd>'+escHtml(r.agentName)+'</dd><dt>'+escHtml(I.searchEmail)+'</dt><dd>'+escHtml(r.ownerEmail||root.dataset.email)+'</dd><dt>'+escHtml(I.exactId)+'</dt><dd>'+escHtml(r.agentId)+'</dd></dl><div class="security-notice"><strong>'+escHtml(I.securityTitle)+'</strong><br>'+escHtml(I.security)+'</div></div>'}
  start();
})();
</script>`;
}
