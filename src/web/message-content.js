...
function createMessageRenderer(labels) {
  ...
  function safeUrl(value) {
    ...
    if (/^\/api\/e2ee-v2\/attachments\/[A-Za-z0-9_-]{8,128}\?agentId=[A-Za-z0-9._-]{1,128}$/.test(raw)) return raw;
    try {
  ...
  function paperclipIcon() {
    return '<svg class="voko-paperclip-icon" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>';
  }
  ...
  function normalize(contentType, content) {
    ...
    const attachment = payload && rawUrl && ['name', 'fileName', 'file_name', 'size', 'fileSize', 'file_size', 'type', 'mimeType', 'mime_type']
    ...
    if (attachment) {
      ...
      if ((mime && mime.toLowerCase().startsWith('image/')) || imageName) {
        return {
          kind: 'image', url, name: String(name), size: formatSize(size),
  ...
  function renderMedia(contentType, content) {
    ...
    if (media.kind === 'image') {
      const name = media.name || text.image;
      const meta = [media.size, media.mime].filter(Boolean).join(' · ');
      const body = '<span class="voko-file-icon" aria-hidden="true">🖼️</span><span class="voko-file-info"><strong class="voko-file-name">' + esc(name) + '</strong>' + (meta ? '<small class="voko-file-meta">' + esc(meta) + '</small>' : '') + '</span>';
      if (!media.url) return '<div class="voko-file-card">' + body + '<span class="voko-resource-unavailable">' + esc(text.unavailable) + '</span></div>';
      return '<a class="voko-file-card voko-image-file-link" href="' + esc(media.url) + '" target="_blank" rel="noopener noreferrer" title="' + esc(text.openOriginal) + '">' + body + '<span class="voko-file-action">' + esc(text.openOriginal) + '</span></a>';
    }

    const isVideo = media.kind === 'video';
    const icon = isVideo ? '🎬' : paperclipIcon();
  ...
  function preview(contentType, content, maxLength = 60) {
    const media = normalize(contentType, content);
    if (media) {
      if (media.kind === 'image') return '🖼️ ' + String(media.name || text.image);
      if (media.kind === 'video') return '🎬 ' + String(text.video);
      return '📎 ' + String(media.name || text.file || text.unknownFile);
    }
    const raw = String(content == null ? '' : content);
    return raw.length > maxLength ? raw.substring(0, maxLength) + '…' : raw;
  }

  function previewHtml(contentType, content, maxLength = 60) {
    const media = normalize(contentType, content);
    if (media && media.kind === 'file') {
      return '<span class="voko-message-preview-media">' + paperclipIcon() + '<span>' + esc(media.name || text.file || text.unknownFile) + '</span></span>';
    }
    return esc(preview(contentType, content, maxLength));
  }
  ...
  return { normalize, preview, previewHtml, renderMedia, render, renderExpandable };
}
