  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        var actionHtml='<a href="/agents/'+esc(a.agentId)+'/edit" class="btn btn-sm btn-outline" style="margin:1px;padding:1px 6px;font-size:11px;min-height:auto">'+L('common.btn.edit')+'</a> <span class="'+(a.publishStatus==='published'?'online':'pending')+'" data-role="toggle-pub" data-agent="'+esc(a.agentId)+'" data-pub-status="'+(a.publishStatus==='published'?'published':'unpublished')+'" title="'+esc(a.publishStatus==='published'?T('common.pub.title_published'):T('common.pub.title_unpublished'))+'" style="cursor:pointer;font-size:12px">'+L(a.publishStatus==='published'?'common.pub.published':'common.pub.unpublished')+'</span> <span class="'+(a.accessMode==='private'?'online':'pending')+'" data-role="toggle-acc" data-agent="'+esc(a.agentId)+'" data-acc-mode="'+(a.accessMode==='private'?'private':'public')+'" title="'+esc(a.accessMode==='private'?T('common.acc.title_private'):T('common.acc.title_public'))+'" style="cursor:pointer;font-size:12px">'+L(a.accessMode==='private'?'common.acc.private':'common.acc.public')+'</span>';
        rows.push('<tr data-agent-id="'+esc(a.agentId)+'"><td class="home-agent-name"><a href="/agents/'+esc(a.agentId)+'"'+agentNameHint+'>'+esc(agentNameDisplay)+'</a></td><td style="white-space:nowrap;font-size:14px;text-align:center">'+esc(bt)+'</td><td data-role="connection-status" style="white-space:nowrap;font-size:14px;text-align:center">'+connStatus+'</td><td data-role="message-mode" data-message-mode-detected="'+(messageModeDetected?'true':'false')+'" style="white-space:nowrap;font-size:14px;text-align:center">'+esc(messageMode)+'</td><td class="home-agent-short" style="font-size:13px">'+shortCell+'</td><td class="home-agent-actions" style="white-space:normal;font-size:13px;text-align:center;line-height:1.9">'+actionHtml+'</td></tr>');        jd.push({name:a.agentName,identifier:a.agentId})
      }
      ...
      const body='<div class="info-bar" style="display:flex;flex-wrap:nowrap;justify-content:space-between;align-items:center;gap:8px">'
        ...
        +'</div>'
        +(filtered.length>0
          ?'<style>.home-agent-table{table-layout:fixed;min-width:0}.home-agent-name a,.home-agent-short a{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.home-agent-actions .btn,.home-agent-actions span{white-space:nowrap}@media(max-width:760px){.home-agent-table{min-width:760px}}</style><div class="table-wrap"><table class="home-agent-table"><colgroup><col style="width:21%"><col style="width:12%"><col style="width:9%"><col style="width:13%"><col style="width:18%"><col style="width:27%"></colgroup><thead><tr><th style="text-align:center">'+L('web.home.col.agent')+'</th><th style="text-align:center">'+L('web.home.col.type')+'</th><th style="text-align:center">'+L('web.home.col.status')+'</th><th style="text-align:center">'+L('web.home.col.message_mode')+'</th><th style="text-align:center">'+L('web.home.col.short_link')+'</th><th style="text-align:center">'+L('web.home.col.actions')+'</th></tr></thead><tbody>'+rows.join('\n')+'</tbody></table></div>'
