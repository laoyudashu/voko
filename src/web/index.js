  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        if(db){try{var sr=db.prepare('SELECT short_link_url FROM agents WHERE agent_id=?').get(a.agentId);if(sr&&sr.short_link_url){var su=esc(sr.short_link_url);shortCell='<span class="home-short-link"><a href="'+su+'" target="_blank" style="font-size:13px">'+su.substring(0,35)+(su.length>35?'…':'')+'</a><button class="btn btn-sm btn-outline" data-role="copy-link" data-url="'+su+'" style="margin:0;padding:1px 6px;font-size:11px;min-height:auto">'+L('common.btn.copy')+'</button></span>'}}catch(ex){}}
        var actionHtml='<a href="/agents/'+esc(a.agentId)+'/edit" class="btn btn-sm btn-outline" style="margin:1px;padding:1px 6px;font-size:11px;min-height:auto">'+L('common.btn.edit')+'</a> <button type="button" class="btn btn-sm home-mode-toggle '+(a.publishStatus==='published'?'home-mode-published':'home-mode-unpublished')+'" data-role="toggle-pub" data-agent="'+esc(a.agentId)+'" data-pub-status="'+(a.publishStatus==='published'?'published':'unpublished')+'" title="'+esc(a.publishStatus==='published'?T('common.pub.title_published'):T('common.pub.title_unpublished'))+'">'+L(a.publishStatus==='published'?'common.pub.published':'common.pub.unpublished')+'</button> <button type="button" class="btn btn-sm home-mode-toggle '+(a.accessMode==='private'?'home-mode-private':'home-mode-public')+'" data-role="toggle-acc" data-agent="'+esc(a.agentId)+'" data-acc-mode="'+(a.accessMode==='private'?'private':'public')+'" title="'+esc(a.accessMode==='private'?T('common.acc.title_private'):T('common.acc.title_public'))+'">'+L(a.accessMode==='private'?'common.acc.private':'common.acc.public')+'</button>';
