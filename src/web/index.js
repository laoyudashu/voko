...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/external-integrations',async(req,res)=>{
    ...
    const rows=integrations.map(item=>'<tr><td><strong>'+esc(item.name||'-')+'</strong><div class="meta" title="'+esc(item.integration_id||'')+'">'+esc(String(item.integration_id||'').slice(0,18))+(String(item.integration_id||'').length>18?'…':'')+'</div></td><td style="max-width:320px;white-space:normal;word-break:break-all">'+esc(item.webhook_url||'-')+'</td><td><code>'+esc(item.token_prefix||'-')+'…</code></td><td>'+esc(fmtTime(item.last_used_at)||T('web.external_gateway.never_used'))+'</td><td style="text-align:right"><button type="button" class="btn-sm btn-danger" data-role="revoke-external-integration" data-integration="'+esc(item.integration_id||'')+'" data-name="'+esc(item.name||'')+'" style="margin:0;padding:5px 12px;min-height:auto">'+L('web.external_gateway.revoke')+'</button></td></tr>').join('');
    const list=loadError?'<div class="card error">'+esc(loadError)+'</div>':rows?'<div class="table-wrap"><table style="min-width:760px"><thead><tr><th>'+L('web.external_gateway.col.name')+'</th><th>'+L('web.external_gateway.col.webhook')+'</th><th>'+L('web.external_gateway.col.token')+'</th><th>'+L('web.external_gateway.col.last_used')+'</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="card"><p class="meta" style="margin:0">'+L('web.external_gateway.empty')+'</p></div>';
