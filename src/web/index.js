...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/external/:principalViewId',async(req,res)=>{
    ...
    try{
      ...
      const contexts=Array.from(rows.reduce((map,row)=>{const id=String(row.context_id||'');if(!map.has(id))map.set(id,[]);map.get(id).push(row);return map},new Map()).entries())
        .map(([id,tasks])=>({id,tasks:tasks.sort((a,b)=>new Date(a.created_at||a.updated_at||0)-new Date(b.created_at||b.updated_at||0)),updatedAt:Math.max(...tasks.map(item=>new Date(item.updated_at||0).getTime()))})).sort((a,b)=>b.updatedAt-a.updatedAt);
      const selectedContext=contexts.find(item=>item.id===String(req.query.contextId||''))||contexts[0],details=[];
      for(const row of selectedContext.tasks){const task=await opts.a2aMailboxClient.getInboundTask(row.task_id);if(!task||task.source_channel!=='rest_webhook'||String(task.local_agent_id)!==agentId||String(task.principal_view_id||'')!==principalViewId)return notFound();details.push(task)}
      const agent=await getAgentInfo(handlers,agentId);
      const tabs=contexts.length>1?'<style>'+CONVERSATION_TAB_CSS+'</style><div class="conversation-tab-shell"><div class="conversation-tab-rail" role="tablist">'+contexts.map((context,index)=>'<a class="conversation-tab-card'+(context.id===selectedContext.id?' active':'')+'" role="tab" aria-selected="'+(context.id===selectedContext.id)+'" href="/agents/'+encodeURIComponent(agentId)+'/external/'+encodeURIComponent(principalViewId)+'?contextId='+encodeURIComponent(context.id)+'">'+esc(T('web.agent.external.conversation',{index:index+1}))+'</a>').join('')+'</div></div>':'';
      const messages=details.map(task=>renderA2ATaskConversation(task,systemName,T)).join('');
      const body='<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px"><div><h1 style="margin:0;font-size:22px">'+esc(systemName)+'</h1><p class="meta" style="margin:4px 0 0">'+L('web.agent.tab.external_messages')+'</p></div><a class="btn-sm btn-outline" href="/external-integrations?agentId='+encodeURIComponent(agentId)+'">'+L('web.agent.external.config')+'</a></div>'+tabs+'<section class="card">'+messages+'</section>';
      res.send(renderPage(req,T('web.agent.external.conversation_title',{name:systemName}),body,{showTitle:false,nav:agentNav(agentId,agent?.agentName||agentId,T)+' › '+esc(systemName),footer:renderFooter(T,locale)}));
  ...
  R.get('/agents/:agentId',async(req,res,next)=>{
    try{
      ...
      const externalLabel=L('web.agent.tab.external_messages')+(externalGroups.length?' ('+externalGroups.length+')':'');
