...
function openOwnerLinkDialog(t){var dlg=ownerLinkDialog(),aid=t.dataset.agent||"",saved=readOwnerLinks()[aid]||{};dlg.dataset.agent=aid;dlg.querySelector("#owner-link-url").value=saved.ownerUrl||"";dlg.querySelector("#owner-link-expiry").textContent=saved.expiresAt?I.owner_expires+new Date(saved.expiresAt).toLocaleString():"";dlg.showModal()}
function createOwnerLink(t){var dlg=ownerLinkDialog(),aid=dlg.dataset.agent||"",original=t.textContent;if(!aid)return;t.disabled=true;t.textContent=I.gen_creating;fetch("/api/owner-link/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId:aid})}).then(function(r){return r.json().then(function(d){return{ok:r.ok,data:d}})}).then(function(x){if(!x.ok||!x.data.success)throw new Error(x.data.error||I.gen_failed);var d=x.data.data||{},input=dlg.querySelector("#owner-link-url"),expiry=dlg.querySelector("#owner-link-expiry");saveOwnerLink(aid,d);renderOwnerLinkEntry(aid,d);input.value=d.ownerUrl||"";expiry.textContent=d.expiresAt?I.owner_expires+new Date(d.expiresAt).toLocaleString():""}).catch(function(e){var m=document.getElementById("toast-msg");if(m)m.textContent=e.message||I.gen_failed;var errorDialog=document.getElementById("dlg-toast");if(errorDialog)errorDialog.showModal()}).finally(function(){t.disabled=false;t.textContent=original})}
restoreOwnerLinks();
(function(){
  var ws = null;
...
document.addEventListener("click",function(e){var t=e.target.closest?e.target.closest("[data-role]")||e.target:e.target;
 ...
 else if(t.matches("[data-role=gen-owner-link]")){openOwnerLinkDialog(t)}
 else if(t.matches("[data-role=confirm-owner-link]")){createOwnerLink(t)}
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        var shortCell='<div class="home-access-stack"><div class="home-access-row home-access-visitor-row"><span class="home-access-label">'+accessIcon('visitor')+L('web.home.access.visitor')+'</span>'+visitorValue+visitorAction+accessModeButton+'</div><div class="home-access-row" data-owner-agent="'+esc(a.agentId)+'"><span class="home-access-label">'+accessIcon('owner')+L('web.home.access.owner')+'</span><span class="home-access-value" data-owner-link-value>'+L('web.home.access.owner_desc')+'</span><span class="home-access-action" data-owner-link-actions><button class="btn btn-sm btn-outline home-owner-action" data-role="gen-owner-link" data-agent="'+esc(a.agentId)+'">'+L('common.btn.generate_link')+'</button></span></div><div class="home-access-row"><span class="home-access-label">'+accessIcon('a2a')+'A2A</span>'+a2aValue+a2aAction+'</div></div>';
...
try{const r=await handlers.list_access_lists({agentId,listType:'whitelist',limit,offset,keyword});const es=r.data||r.entries||r.accessList||[],total=r.total||es.length;totalPages=Math.ceil(total/limit);let wlNickMap={};if(es.length){try{const vids=es.map(e=>e.visitor_id||e.visitorId||'').filter(Boolean);if(vids.length){const rows=db.prepare('SELECT uid, nickname FROM user_cache WHERE uid IN ('+vids.map(()=>'?').join(',')+')').all(...vids);rows.forEach(r=>{wlNickMap[r.uid]=r.nickname||'';});}}catch(_){}const wlName=(vid)=>{const n=wlNickMap[vid];return n?esc(n)+' ('+esc(vid)+')':esc(vid);};const wlSource=(e)=>e.source==='same_owner_default'?(e.auto_trust_disabled?'已关闭自动信任':'同一主人自动信任'):(e.source==='outbound_contact'?'主动联系自动加入':'手动添加');listHtml='<div class="table-wrap"><table><thead><tr><th>'+L('web.agent.whitelist.col.visitor')+'</th><th>来源</th><th>'+L('web.agent.whitelist.col.reason')+'</th><th style="text-align:center">'+L('web.agent.whitelist.col.action')+'</th></tr></thead><tbody>'+es.map(e=>'<tr><td>'+wlName(e.visitor_id||e.visitorId||'')+'</td><td>'+esc(wlSource(e))+'</td><td>'+esc(e.reason||'-')+'</td><td style="text-align:center"><form method="POST" action="/agents/'+esc(agentId)+'" data-voko-access-list style="display:inline"><input type="hidden" name="_action" value="remove_whitelist"><input type="hidden" name="visitorId" value="'+esc(e.visitor_id||e.visitorId||'')+'"><button type="submit" class="btn-xs btn-outline" style="margin:0;padding:2px 8px;font-size:11px;min-height:auto">'+L('common.btn.remove')+'</button></form></td></tr>').join('\n')+'</tbody></table></div>'}}catch{}
  ...
  R.get('/agents/:agentId/caps',async(req,res,next)=>{
    try{
      ...
      let a2aPublic=true;
      try{opts.a2aModule.withDatabase(function(a2aDb){const{A2APublicationStore}=require('../a2a');const stored=new A2APublicationStore(a2aDb).getPublicEnabled(agentId);a2aPublic=stored===null?true:stored})}catch{}
