  ...
  R.get('/agents/:agentId/c/:channelId',async(req,res,next)=>{
    try{
      ...
      if(webRoutingEnabled('web_private_conversations_v1')){
        ...
        const historyVisible=showLegacyHistory||(webConversations.length>1&&unboundMessageCount>0);
        const history=historyVisible?'<a class="conversation-tab-card'+(showLegacyHistory?' active':'')+'" role="tab" href="/agents/'+esc(agentId)+'/c/'+esc(channelId)+'?history=1">'+L('web.conversation.tab.history')+'</a>':'';
