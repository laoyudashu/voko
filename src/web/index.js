...
function createWebRouter(handlers, db, opts={}){
  ...
  R.post('/api/agents/:agentId/provider-security/refresh',requireSensitiveLocalAuth,requireSensitiveCsrf,async(req,res)=>{
    try{
      ...
      const data=await opts.dispatcher.refreshProviderSecurityCapability(String(req.params.agentId));
      res.json({success:true,data});
    }catch(error){res.status(400).json({success:false,error:String(error.message||error)})}
  });
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const form=hasVisibleControls?'<form id="provider-security-form" data-transport="'+esc(data.transportId)+'" data-agent-name="'+esc(data.agentName)+'">'+hermesPreview+rows+(visibleControls.some(item=>item.editable)?confirmation+'<button type="submit">'+(visualSecurityTransports.has(data.transportId)?(zh?'保存设置':'Save settings'):(zh?'预检并保存':'Preflight & save'))+'</button>':'')+'<p id="provider-security-result" class="meta" hidden></p></form>':'<div class="card"><h2>'+esc(unavailableTitle)+'</h2><p><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</p><p class="meta">'+esc(unavailable)+'</p></div>';
      ...
      const script='<script>(function(){var form=document.getElementById("provider-security-form");if(!form)return;var out=document.getElementById("provider-security-result"),confirmBox=document.getElementById("provider-security-confirmation"),confirmInput=document.getElementById("provider-security-confirmation-input"),L='+jsonForInlineScript(labels)+';function show(text,kind){out.hidden=false;out.textContent=text;out.className=kind||"meta"}function updateRisk(field){var option=field.tagName==="SELECT"?field.options[field.selectedIndex]:null,risk=option&&option.dataset.risk||field.dataset.risk||"low",badge=field.parentElement.querySelector("[data-risk-indicator]");if(badge){badge.textContent=risk==="high"?'+jsonForInlineScript(zh?'高风险':'High risk')+':risk==="medium"?'+jsonForInlineScript(zh?'中等风险':'Medium risk')+':'+jsonForInlineScript(zh?'低风险':'Low risk')+';badge.className="badge "+(risk==="high"?"badge-offline":risk==="medium"?"badge-pending":"badge-online")}var count=field.parentElement.querySelector("[data-char-count]");if(count)count.textContent=field.value.length+" / "+(field.maxLength||2000)}function updatePreview(){var command=document.getElementById("provider-command-preview");if(!command)return;var transport=form.dataset.transport,get=function(id){var el=form.querySelector("[data-control="+id+"]");return el?el.value:""};function base(text){command.textContent=text}function addFlag(text,risk){command.appendChild(document.createTextNode(" "));var span=document.createElement("span");span.textContent=text;span.style.display="inline-block";span.style.padding="1px 5px";span.style.borderRadius="5px";span.style.fontWeight="600";if(risk==="high"){span.style.color="#b42318";span.style.background="#fdecec";span.style.border="1px solid #f4b8b3"}else if(risk==="medium"){span.style.color="#a85b00";span.style.background="#fff4df";span.style.border="1px solid #f2cf91"}else{span.style.color="#1769aa";span.style.background="#e8f1fb";span.style.border="1px solid #b8d2ee"}command.appendChild(span)}if(transport==="hermes-cli"){base("hermes --profile <当前 Profile> chat -q <访客消息> -Q --source tool");if(get("toolProfile")==="safe")addFlag("--toolsets safe","medium");if(get("safeMode")==="enabled")addFlag("--safe-mode","medium");if(get("approvalMode")==="bypass")addFlag("--yolo","high");if(get("acceptHooks")==="enabled")addFlag("--accept-hooks","high")}else if(transport==="workbuddy-http"){base("codebuddy --serve --host 127.0.0.1 --permission-mode");addFlag("dontAsk","medium");if(get("dataFileAccess")==="read"){addFlag("--tools Read","high");addFlag("--allowedTools Read(<绑定文件>)（仅自动审批，非路径隔离）","high")}else addFlag("--tools <空列表>","low");if(get("sessionPersistence")==="ephemeral")addFlag("--no-session-persistence","low");addFlag(get("mcpProfile")==="user"?"加载用户 MCP 配置":"--strict-mcp-config",get("mcpProfile")==="user"?"high":"low")}else if(transport==="qwen-office-cli"){base("qoderclicn --print --permission-mode");addFlag(get("permissionMode")||"dont_ask",get("permissionMode")==="bypass_permissions"?"high":"low");addFlag(get("toolAccess")==="default"?"--tools default":get("toolAccess")==="read_only"?"--tools Read,Grep,Glob":"--tools <空列表>",get("toolAccess")==="default"?"high":get("toolAccess")==="read_only"?"medium":"low");if(get("sessionPersistence")==="ephemeral")addFlag("--no-session-persistence","low");addFlag(get("mcpProfile")==="user"?"加载用户 MCP 配置":"--strict-mcp-config --mcp-config {"+String.fromCharCode(34)+"mcpServers"+String.fromCharCode(34)+":{}}",get("mcpProfile")==="user"?"high":"low")}else if(transport==="dumate-http"){base("POST /session/<sessionId>/prompt_async");addFlag(get("sessionPersistence")==="ephemeral"?"每条消息新建 Session":"复用当前访客 Session",get("sessionPersistence")==="ephemeral"?"low":"medium")}}form.querySelectorAll("[data-control]").forEach(function(field){updateRisk(field);field.addEventListener(field.tagName==="TEXTAREA"?"input":"change",function(){updateRisk(field);updatePreview()})});updatePreview();form.addEventListener("submit",async function(event){event.preventDefault();var button=form.querySelector("button[type=submit]"),config={};form.querySelectorAll("[data-control]").forEach(function(field){config[field.dataset.control]=field.value});button.disabled=true;show(L.saving,"meta");try{var pre=await fetch("/api/agents/"+encodeURIComponent('+jsonForInlineScript(agentId)+')+"/provider-security/preflight",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transportId:form.dataset.transport,config:config})}),p=await pre.json();if(!pre.ok||!p.success)throw new Error(p.error||L.failed);var confirmation="";if(p.data.requiresTypedConfirmation){confirmation=confirmInput?confirmInput.value.trim():"";if(!confirmation){if(confirmBox)confirmBox.hidden=false;if(confirmInput)confirmInput.focus();show(L.confirm,"error");return}}var commit=await fetch("/api/agents/"+encodeURIComponent('+jsonForInlineScript(agentId)+')+"/provider-security/commit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({preflightToken:p.data.preflightToken,confirmation:confirmation})}),c=await commit.json();if(!commit.ok||!c.success)throw new Error(c.error||L.failed);if(confirmBox)confirmBox.hidden=true;if(confirmInput)confirmInput.value="";show(L.saved,"success")}catch(error){var message=error.message||L.failed;show(message==="PROVIDER_SECURITY_CONFIRMATION_MISMATCH"?L.confirmationMismatch:message,"error")}finally{button.disabled=false}})})();</script>';
