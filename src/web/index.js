...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const securityTransport=data.transportId?'<span style="display:inline-flex;align-items:center;gap:12px;white-space:nowrap"><span><strong>'+(zh?'安全适配器':'Security adapter')+':</strong> <code>'+esc(data.transportId)+'</code></span>'+transportPicker+'</span>':transportPicker;
      const supportedHeader='<div class="info-bar"><span><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</span><span><strong>'+(zh?'消息推送模式':'Message delivery mode')+':</strong> '+esc(modeLabels[deliveryMode]||deliveryMode)+'</span>'+securityTransport+'</div>';
