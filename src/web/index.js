...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const initialPreviewHtml=initialPreview.map((item,index)=>(index?' ':'')+'<span style="display:inline-block;padding:1px 5px;border-radius:5px;font-weight:'+(item.changed?'600':'400')+';color:'+previewColor(item)+';background:'+previewBackground(item)+'">'+esc(item.text)+'</span>').join('');
      ...
      const transportOptions=(data.transports||[]).filter(item=>item.configured).map(item=>'<option value="'+esc(item.transportId)+'"'+(item.transportId===data.transportId?' selected':'')+'>'+esc((modeLabels[item.mode]||item.mode)+' · '+item.transportId+(item.status==='delivery_stalled'?(zh?' · 提交停滞':' · delivery stalled'):item.automaticReady?' · ready':' · '+(item.status||'unverified')))+'</option>').join('');
      ...
      const deliverySecurityMeta='<span data-provider-delivery-security style="display:inline-flex;align-items:center;gap:14px;white-space:nowrap"><span><strong>'+(zh?'消息推送模式':'Message delivery mode')+':</strong> '+esc(modeLabels[deliveryMode]||deliveryMode)+'</span>'+securityTransport+transportPicker+'</span>';
      const supportedHeader='<div class="info-bar" style="align-items:center"><span><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</span>'+deliverySecurityMeta+'</div>';
