...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/c/:channelId',async(req,res,next)=>{
    try{
      ...
      if(msgs.length){mh='';const s=[...msgs].reverse();for(const m of s){const sr=m.isMe?L('web.conversation.from.agent'):L('web.conversation.from.visitor');const t=timeTag(m.timestamp);if(m.contentType===11){const audit=parseAuditContent(m.content);mh+=renderAuditContent(m.content,T,t);jd.push({from:'system',content:audit.valid?audit.text:T('web.audit.message_invalid'),timestamp:m.timestamp});continue;}const c=messageRenderer.render(m.contentType,m.content);mh+='<div style="padding:8px 12px;margin:4px 0;border-radius:6px;border-left:4px solid '+(m.isMe?'#0f9d58':'#1a73e8')+';background:'+(m.isMe?'#e6f4ea':'#e8f0fe')+'"><strong>'+sr+'</strong> <span style="color:#888;font-size:13px">['+t+']</span><br>'+c+'</div>';jd.push({from:m.isMe?'agent':'visitor',content:m.content,timestamp:m.timestamp})}}
      const aId2=esc(agentId),cId2=esc(channelId);
      // 访客昵称（user_cache 有则显示名称，无则仅显示 id）
      let visitorName=channelId;
      try{const r=db.prepare('SELECT nickname FROM user_cache WHERE uid=? LIMIT 1').get(channelId);if(r&&r.nickname)visitorName=r.nickname;}catch(_){}
    ...
    try{
      ...
      res.send(renderFormPage(T('web.agent.invite.title'),agentId,agent.agentName||agentId,
        actionForm(agentId,'invite',[
          ...
          {id:'fn',name:'friendName',label:T('web.agent.invite.name_opt'),type:'text'},
    ...
    try{
      ...
      try{
        switch(a){
          ...
          case'invite':await handleAction(req,res,handlers.invite_friend({agentId,friendEmail:req.body.friendEmail,friendName:req.body.friendName||''}),'common.action.invite_sent');break;
...
footer:'<script>(function(){try{var ws=new WebSocket("ws://"+location.host+"/ws");ws.onmessage=function(e){try{var d=JSON.parse(e.data);if(d.event==="owner-intervention:email-reply"||d.event==="owner-intervention:new")setTimeout(function(){location.reload()},800)}catch(_){}};ws.onclose=function(){setTimeout(arguments.callee,3000)}}catch(_){}})();</script>'}))
  ...
  R.get('/invite',async(req,res,next)=>{
    try{
      ...
      res.send(renderPage(req,T('web.invite.title'),'<div class="card"><form method="POST" action="/invite" data-agent-action="invite.send"><label for="ia">'+L('web.invite.select_agent')+'</label><select id="ia" name="agentId" required autofocus><option value="">'+L('web.invite.select_ph')+'</option>'+opts+'</select><label for="ie">'+L('web.invite.email')+'</label><input type="email" id="ie" name="friendEmail" required placeholder="friend@example.com" autocomplete="email"><label for="in">'+L('web.invite.name_opt')+'</label><input type="text" id="in" name="friendName" autocomplete="given-name"><br><br><button type="submit">'+L('web.invite.send_btn')+'</button></form></div><a href="/">← '+L('common.btn.home')+'</a>',{nav:'<a href="/">'+L('common.nav.home')+'</a> › '+L('web.invite.breadcrumb')}))
    }catch(e){next(e)}
  });
  R.post('/invite',async(req,res,next)=>{
    try{
      const T=req.t,L=k=>esc(T(k));
      const r=await handlers.invite_friend({agentId:req.body.agentId,friendEmail:req.body.friendEmail,friendName:req.body.friendName||''});
      if(r.success!==false)res.send(renderPage(req,T('web.invite.sent_title'),'<p class="success">'+T('web.invite.sent_msg',{email:esc(req.body.friendEmail)})+'</p><a href="/">← '+L('common.btn.home')+'</a>',{nav:'<a href="/">'+L('common.nav.home')+'</a> › '+L('web.invite.breadcrumb')}));
