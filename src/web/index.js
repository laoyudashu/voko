...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        var visitorValue='<span class="home-access-value" style="font-size:13px">'+L('web.home.access.not_generated')+'</span>',visitorAction='<button class="btn btn-sm btn-outline home-access-action" data-role="gen-link" data-agent="'+esc(a.agentId)+'">'+L('common.btn.generate_link')+'</button>';
        ...
        rows.push('<tr data-agent-id="'+esc(a.agentId)+'"><td class="home-agent-name"><a href="/agents/'+esc(a.agentId)+'"'+agentNameHint+'>'+esc(agentNameDisplay)+'</a></td><td style="white-space:nowrap;font-size:14px;text-align:center">'+esc(bt)+'</td><td data-role="connection-status" style="white-space:nowrap;font-size:14px;text-align:center">'+connStatus+'</td><td data-role="message-mode" data-message-mode-detected="'+(messageModeDetected?'true':'false')+'" style="white-space:nowrap;font-size:14px;text-align:center">'+esc(messageMode)+'</td><td class="home-agent-short'+(agentOnline===false?' is-agent-offline':'')+'" data-agent-online="'+(agentOnline===null?'unknown':agentOnline?'true':'false')+'" style="font-size:13px">'+shortCell+'</td><td class="home-agent-actions" style="white-space:nowrap;font-size:13px;text-align:center">'+actionHtml+'</td></tr>');        jd.push({name:a.agentName,identifier:a.agentId})
      }
  ...
  R.get('/trusted-remote',async(req,res,next)=>{
    try{
      ...
      res.send(renderPage(req,T('web.trusted_remote.title'),body,{footer:renderFooter(T,req.locale)+script}));
  ...
  const ownerChatLiveScript=(agentId,selector,conversationId='')=>'<script>(function(){var aid='+jsonForInlineScript(agentId)+',cid='+jsonForInlineScript(conversationId)+',selector='+jsonForInlineScript(selector)+',busy=false,timer=null;function refresh(){if(busy)return;busy=true;fetch(location.href,{headers:{"Accept":"text/html"},cache:"no-store"}).then(function(r){if(!r.ok)throw new Error("refresh failed");return r.text()}).then(function(html){var doc=new DOMParser().parseFromString(html,"text/html"),fresh=doc.querySelector(selector),current=document.querySelector(selector);if(fresh&&current)current.replaceWith(fresh)}).catch(function(){}).finally(function(){busy=false})}function queue(){clearTimeout(timer);timer=setTimeout(refresh,120)}function connect(){try{var protocol=location.protocol==="https:"?"wss://":"ws://",ws=new WebSocket(protocol+location.host+"/ws");ws.onmessage=function(e){try{var d=JSON.parse(e.data),data=d.data||{};if(d.event==="owner-chat:updated"&&data.agentId===aid&&(!cid||!data.conversationId||data.conversationId===cid))queue()}catch(_){}};ws.onclose=function(){setTimeout(connect,3000)}}catch(_){setTimeout(connect,5000)}}connect()})();</script>';
  ...
  R.get('/agents/:agentId',async(req,res,next)=>{
    try{
      ...
      const requestedTab=req.query.tab==='group'?'group':req.query.tab==='owner'?'owner':'conv';
      ...
      const tabBar='<div style="display:flex;gap:4px;border-bottom:2px solid #e0e0e0;margin-bottom:14px">'+tabBtn('conv',convLabel,activeTab==='conv')+tabBtn('group',groupLabel,activeTab==='group')+(ownerTotal?tabBtn('owner',ownerLabel,activeTab==='owner'):'')+'</div>';
  ...
  R.get('/agents/:agentId/owner-chats/:conversationId',async(req,res,next)=>{
    ...
    try{
      ...
      const bubble=message=>{
        ...
        const owner=message.direction==='owner';return '<article class="owner-chat-bubble '+(owner?'owner':'agent')+'"><div class="owner-chat-sender">'+(owner?L('web.owner_chat.owner_verified'):aName)+'</div><div class="owner-chat-body">'+content+'</div><div class="owner-chat-meta">'+statusLabel(message.state)+' · '+timeTag(message.createdAt)+'</div></article>';
      };
      const body='<div class="owner-chat-detail" data-owner-chat-agent="'+esc(agentId)+'" data-owner-chat-conversation="'+esc(conversationId)+'"><div class="owner-chat-readonly-note">'+L('web.owner_chat.readonly_notice')+'</div><div class="owner-chat-transcript">'+messages.map(bubble).join('')+'</div></div><p><a href="/agents/'+esc(agentId)+'?tab=owner">← '+L('web.owner_chat.back')+'</a></p><style>.owner-chat-readonly-note{padding:10px 12px;margin-bottom:14px;border:1px solid #b9d4fb;border-radius:8px;background:#f4f8ff;color:#345}.owner-chat-transcript{display:flex;flex-direction:column;gap:12px}.owner-chat-bubble{max-width:78%;padding:10px 12px;border-radius:12px}.owner-chat-bubble.owner{align-self:flex-start;background:#f2f4f7}.owner-chat-bubble.agent{align-self:flex-end;background:#eaf3ff}.owner-chat-sender{font-size:12px;font-weight:700;color:#475467;margin-bottom:5px}.owner-chat-body{word-break:break-word;line-height:1.6}.owner-chat-meta{font-size:11px;color:#8a8f98;margin-top:6px}.owner-chat-image{display:block;max-width:min(100%,520px);max-height:420px;border-radius:8px}.owner-chat-file{display:inline-block}.owner-chat-file-unavailable{color:#8a8f98}.owner-chat-text{margin-top:5px}@media(max-width:640px){.owner-chat-bubble{max-width:92%}}</style>';
      res.send(renderPage(req,T('web.owner_chat.title',{name:agent.agentName||agent.agentId}),body,{nav:agentNav(agentId,agent.agentName||agent.agentId,T),footer:renderFooter(T,req.locale)+ownerChatLiveScript(agentId,'.owner-chat-detail',conversationId)}));
    }catch(error){next(error)}
  });
