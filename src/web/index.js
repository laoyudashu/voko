...
function accessIcon(kind){const path=kind==='visitor'?'<path d="M4 7h16v10H7l-3 3V7Z"/><path d="M8 11h8M8 14h5"/>':kind==='owner'?'<path d="M12 3 5 6v5c0 4.5 2.9 8.1 7 10 4.1-1.9 7-5.5 7-10V6l-7-3Z"/><path d="m9 12 2 2 4-4"/>':kind==='im'?'<rect x="3" y="5" width="18" height="14" rx="2"/><circle cx="8" cy="11" r="2"/><path d="M12 10h5M12 14h5M6 16h4"/>':kind==='external'?'<path d="M8 7h8M8 12h8M8 17h5"/><rect x="3" y="3" width="18" height="18" rx="3"/>':'<circle cx="6" cy="12" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="18" cy="18" r="2"/><path d="m8 11 8-4M8 13l8 4"/>';return '<svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" focusable="false" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'+path+'</svg>'}
...
function positionMessageModeMenu(details){var menu=details.querySelector("[data-role=message-mode-menu]");details.classList.remove("is-dropup");if(!details.open||!menu)return;var menuRect=menu.getBoundingClientRect(),detailsRect=details.getBoundingClientRect(),bottomBoundary=window.innerHeight-8,wrap=details.closest(".table-wrap");if(wrap)bottomBoundary=Math.min(bottomBoundary,wrap.getBoundingClientRect().bottom-8);if(menuRect.bottom>bottomBoundary&&detailsRect.top>menu.offsetHeight+8)details.classList.add("is-dropup")}
function renderMessageModeMenu(details,status){var menu=details.querySelector("[data-role=message-mode-menu]"),preferred=String(status.temporaryPreferredMode||""),preferredProvider=String(status.temporaryPreferredProvider||"");if(!menu)return;menu.innerHTML=(status.methods||[]).map(function(method){var manuallySelected=!!preferred&&method.mode===preferred&&String(method.provider||"")===preferredProvider,activeSelected=!preferred&&method.mode===status.activeAutomaticMode,selected=manuallySelected||activeSelected,enabled=method.available===true,state=manuallySelected?I.message_mode_temporary:(activeSelected?I.message_mode_active:(enabled?I.message_mode_available:I.message_mode_unavailable));return '<button type="button" class="home-message-mode-option'+(selected?' is-selected':'')+'" data-role="select-message-mode" data-mode="'+String(method.mode).replace(/"/g,'&quot;')+'" data-provider="'+String(method.provider||'').replace(/"/g,'&quot;')+'" '+(enabled?'':'disabled')+'><span class="home-message-mode-dot '+(enabled?'is-available':'is-unavailable')+'"></span><span class="home-message-mode-name">'+messageModeLabel(method.mode)+'</span><span class="home-message-mode-state">'+state+'</span></button>'}).join('');requestAnimationFrame(function(){positionMessageModeMenu(details)})}
...
function externalGatewayHomeScript(t){
  const labels={
    configured:t('web.home.access.external_configured',{count:'{count}'}),
    notConfigured:t('web.home.access.external_not_configured'),
    unavailable:t('web.home.access.external_unavailable'),
...
function createWebRouter(handlers, db, opts={}){
  ...
  const renderA2ATaskConversation=(task,counterparty,T=key=>key,options={})=>{
    ...
    const deliveryState=esc(T('web.agent.a2a.delivery.'+String(task?.delivery_state||task?.metadata?.deliveryState||'unknown').toLowerCase()));
    ...
    const taskMeta=options.showTaskMeta
      ? '<div class="a2a-task-card-head"><div class="a2a-task-card-title"><span class="badge badge-online">'+esc(options.taskLabel||T('web.a2a_principal.task_tab',{index:1}))+'</span><a class="a2a-task-card-id" href="/a2a-tasks/'+encodeURIComponent(taskId)+'" title="'+esc(T('web.a2a_task.breadcrumb'))+'">'+esc(taskId)+'</a></div><div class="a2a-task-card-inline-meta"><span><strong>'+esc(T('web.a2a_task.state'))+'</strong> <span class="a2a-task-card-chip success">'+taskState+'</span></span><span><strong>'+esc(T('web.a2a_task.delivery'))+'</strong> <span class="a2a-task-card-chip info">'+deliveryState+'</span></span><span class="a2a-task-card-message-count">'+compactMessageCount+'</span></div></div>'
  ...
  R.get('/agents/:agentId/a2a/:principalViewId',async(req,res)=>{
    ...
    try{
      ...
      const taskCards=details.map((task,index)=>'<section class="card a2a-task-card" id="task-'+esc(task.gateway_task_id)+'">'+renderA2ATaskConversation(task,principalDisplayId,T,{showTaskMeta:true,taskLabel:T('web.a2a_principal.task_tab',{index:index+1})})+'</section>').join('');
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        var visitorValue='<span class="home-access-value" style="font-size:13px">'+L('web.home.access.not_generated')+'</span>',visitorAction='<button class="btn btn-sm btn-outline home-access-action" data-role="gen-link" data-agent="'+esc(a.agentId)+'">'+L('common.btn.generate_link')+'</button>',accessRow=null;
        ...
        const a2aValue='<span class="home-access-value" style="font-size:13px">'+L(a2a?.published?'web.home.access.a2a_published':'web.home.access.a2a_unpublished')+'</span>';
        const a2aAction=a2a?.published?copyButton({esc,label:L('web.home.access.copy_a2a'),attrs:'data-voko-copy-value="'+esc(a2aCard)+'"'}):'<a class="btn btn-sm btn-outline home-access-action home-a2a-action" href="/agents/'+esc(a.agentId)+'/caps">'+L('web.home.access.setup')+'</a>';
        const imUid=String(accessRow&&accessRow.imUid||''),imUidValue='<span class="home-access-value" data-role="im-uid-value" title="'+esc(imUid)+'">'+esc(imUid||'-')+'</span>',imUidAction=imUid?copyButton({esc,label:L('web.home.access.copy_im_uid'),attrs:'data-voko-copy-value="'+esc(imUid)+'"'}):'';
        const externalPublicAgentId=String(a2a?.publicId||a.agentId||'');
        const externalValue='<span class="home-access-value" data-role="external-integration-value">'+L('web.home.access.external_loading')+'</span>';
        const externalAction='<a class="btn btn-sm btn-outline home-access-action" data-role="external-integration-action" href="/external-integrations?agentId='+encodeURIComponent(a.agentId)+'">'+L('web.home.access.external_manage')+'</a>';
        ...
        var shortCell='<div class="home-access-stack"><div class="home-access-row home-access-visitor-row"><span class="home-access-label">'+accessIcon('visitor')+L('web.home.access.visitor')+'</span>'+visitorValue+visitorAction+accessModeButton+'</div><div class="home-access-row"><span class="home-access-label">'+accessIcon('a2a')+'A2A</span>'+a2aValue+a2aAction+'</div><div class="home-access-row"><span class="home-access-label">'+accessIcon('im')+'IM UID</span>'+imUidValue+imUidAction+'</div><div class="home-access-row home-access-external-row" data-role="external-integration-row" data-public-agent-id="'+esc(externalPublicAgentId)+'"><span class="home-access-label">'+accessIcon('external')+L('web.home.access.external')+'</span>'+externalValue+externalAction+'</div></div>';
        ...
        rows.push('<tr data-agent-id="'+esc(a.agentId)+'"><td class="home-agent-name"><a href="/agents/'+esc(a.agentId)+'"'+agentNameHint+'>'+esc(agentNameDisplay)+'</a></td><td style="white-space:nowrap;font-size:14px;text-align:center">'+esc(bt)+'</td><td data-role="connection-status" style="white-space:nowrap;font-size:14px;text-align:center">'+connStatus+'</td><td data-role="message-mode" data-message-mode-detected="'+(messageModeDetected?'true':'false')+'" style="white-space:nowrap;font-size:14px;text-align:center">'+messageModePicker+'</td><td class="home-agent-short'+(agentOnline===false?' is-agent-offline':'')+'" data-agent-online="'+(agentOnline===null?'unknown':agentOnline?'true':'false')+'" style="font-size:13px">'+shortCell+'</td><td class="home-agent-actions" style="white-space:nowrap;font-size:13px;text-align:center">'+actionHtml+'</td></tr>');        jd.push({name:a.agentName,identifier:a.agentId})
      }

      // 信息栏
      const messageModeStyle='<style>.home-message-mode-picker{position:relative;display:inline-block}.home-message-mode-picker summary{list-style:none;cursor:pointer;color:#175cd3;border-radius:6px;padding:2px 18px 2px 6px;position:relative}.home-message-mode-picker summary::-webkit-details-marker{display:none}.home-message-mode-picker summary:after{content:"▾";position:absolute;right:4px;color:#667085}.home-message-mode-picker[open] summary{background:#eff6ff}.home-message-mode-picker.is-agent-offline summary{color:#b0b5bd;cursor:not-allowed;pointer-events:none}.home-message-mode-picker.is-agent-offline summary:after{color:#c8ccd2}.home-message-mode-menu{position:absolute;z-index:30;top:calc(100% + 5px);left:50%;transform:translateX(-50%);width:210px;padding:6px;background:#fff;border:1px solid #d0d5dd;border-radius:10px;box-shadow:0 10px 28px rgba(15,23,42,.16);text-align:left}.home-message-mode-picker.is-dropup .home-message-mode-menu{top:auto;bottom:calc(100% + 5px)}.home-message-mode-option{display:grid;grid-template-columns:10px minmax(0,1fr) auto;align-items:center;gap:7px;width:100%;margin:0;padding:8px;border:0;border-radius:7px;background:transparent;color:#344054;text-align:left;font-size:13px;cursor:pointer}.home-message-mode-option:hover:not(:disabled){background:#f2f7ff}.home-message-mode-option:disabled{cursor:not-allowed;color:#98a2b3}.home-message-mode-option.is-selected{background:#edf9f1}.home-message-mode-dot{width:8px;height:8px;border-radius:50%}.home-message-mode-dot.is-available{background:#12b76a}.home-message-mode-dot.is-unavailable{background:#98a2b3}.home-message-mode-name{font-weight:600}.home-message-mode-state{font-size:11px;color:#667085}.home-message-mode-loading{display:block;padding:9px;color:#667085;font-size:12px;text-align:center}.home-message-mode-loading.is-error{color:#b42318}</style>';
      const body=messageModeStyle+'<div class="info-bar" style="display:flex;flex-wrap:nowrap;justify-content:space-between;align-items:center;gap:8px">'
  ...
  R.get('/external-integrations',async(req,res)=>{
    ...
    const rows=integrations.map(item=>'<tr><td><strong>'+esc(item.name||'-')+'</strong><div class="meta" title="'+esc(item.integration_id||'')+'">'+esc(String(item.integration_id||'').slice(0,18))+(String(item.integration_id||'').length>18?'…':'')+'</div></td><td style="max-width:280px;white-space:normal;word-break:break-all">'+esc(item.webhook_url||'-')+'</td><td>'+credentialCell(item.token,String(item.token_prefix||'-')+'…',T('web.external_gateway.copy_token'),T('web.external_gateway.credential_unavailable'))+'</td><td>'+credentialCell(item.webhookSecret,'-',T('web.external_gateway.copy_secret'),T('web.external_gateway.secret_unavailable'))+'</td><td style="text-align:right"><button type="button" class="btn-sm btn-danger" data-role="revoke-external-integration" data-integration="'+esc(item.integration_id||'')+'" data-name="'+esc(item.name||'')+'" style="margin:0;padding:5px 12px;min-height:auto">'+L('web.external_gateway.revoke')+'</button></td></tr>').join('');
    const list=loadError?'<div class="card error">'+esc(loadError)+'</div>':rows?'<div class="table-wrap"><table style="min-width:900px"><thead><tr><th>'+L('web.external_gateway.col.name')+'</th><th>'+L('web.external_gateway.col.webhook')+'</th><th>'+L('web.external_gateway.col.token')+'</th><th>'+L('web.external_gateway.col.secret')+'</th><th></th></tr></thead><tbody>'+rows+'</tbody></table></div>':'<div class="card"><p class="meta" style="margin:0">'+L('web.external_gateway.empty')+'</p></div>';
    const body='<p class="meta" style="max-width:760px">'+esc(T('web.external_gateway.intro',{agent:agent.agent_name||agent.agent_id}))+'</p>'
      +'<section class="card" style="padding:18px 20px;max-width:760px"><h2 style="margin-top:0">'+L('web.external_gateway.rest_title')+'</h2><p class="meta" style="margin:0 0 10px">'+L('web.external_gateway.rest_help')+'</p><label for="external-rest-endpoint">'+L('web.external_gateway.rest_endpoint')+'</label><div style="display:flex;align-items:center;gap:8px"><input id="external-rest-endpoint" readonly value="'+esc(restEndpoint)+'" style="max-width:none;margin:0;font-family:monospace;font-size:13px"><button type="button" class="btn-sm btn-outline" data-voko-copy-value="'+esc(restEndpoint)+'" style="margin:0;white-space:nowrap">'+L('web.external_gateway.copy_rest')+'</button></div></section>'
      +'<section class="card" style="padding:18px 20px;max-width:760px"><h2 style="margin-top:0">'+L('web.external_gateway.create_title')+'</h2><form id="external-integration-create"><input type="hidden" name="agentId" value="'+esc(agentId)+'"><label for="external-name">'+L('web.external_gateway.name')+'</label><input id="external-name" name="name" maxlength="128" required placeholder="'+L('web.external_gateway.name_placeholder')+'" style="max-width:none"><label for="external-webhook">'+L('web.external_gateway.webhook_url')+'</label><input id="external-webhook" name="webhookUrl" type="url" maxlength="2048" required pattern="https://.*" placeholder="https://crm.example.com/voko/events" style="max-width:none"><p class="meta" style="margin:5px 0 0">'+L('web.external_gateway.webhook_help')+'</p><div id="external-create-error" class="error" style="display:none;margin-top:10px"></div><button type="submit" style="margin-top:14px">'+L('web.external_gateway.create')+'</button></form></section>'
      +'<h2>'+esc(T('web.external_gateway.list_title',{count:integrations.length}))+'</h2>'+list
      +'<dialog id="external-credentials" style="border:none;border-radius:12px;padding:0;max-width:620px;width:calc(100% - 32px);box-shadow:0 12px 36px rgba(15,23,42,.22)"><div style="padding:22px 24px"><h3 style="margin:0 0 8px">'+L('web.external_gateway.credentials_title')+'</h3><p class="meta" style="margin:0 0 14px">'+L('web.external_gateway.credentials_warning')+'</p><label for="external-token">'+L('web.external_gateway.api_token')+'</label><textarea id="external-token" readonly rows="3" style="max-width:none;font-family:monospace"></textarea><label for="external-secret">'+L('web.external_gateway.webhook_secret')+'</label><textarea id="external-secret" readonly rows="3" style="max-width:none;font-family:monospace"></textarea><div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:16px"><button type="button" class="btn-sm btn-outline" data-role="copy-external-credentials" style="margin:0">'+L('web.external_gateway.copy_credentials')+'</button><button type="button" class="btn-sm" data-role="close-external-credentials" style="margin:0">'+L('common.btn.close')+'</button></div></div></dialog>'
    ...
    const labels={restEndpoint,creating:T('web.external_gateway.creating'),createFailed:T('web.external_gateway.create_failed'),copyTemplate:T('web.external_gateway.copy_template'),copied:T('common.home.copied'),revokeMessage:T('web.external_gateway.revoke_message',{name:'{name}'}),revokeFailed:T('web.external_gateway.revoke_failed')};
    const script='<script>(function(){var L='+jsonForInlineScript(labels)+',form=document.getElementById("external-integration-create"),credentials=document.getElementById("external-credentials"),token=document.getElementById("external-token"),secret=document.getElementById("external-secret"),createError=document.getElementById("external-create-error"),revoke=document.getElementById("external-revoke-confirm"),pending="";form.addEventListener("submit",function(e){e.preventDefault();var button=form.querySelector("button[type=submit]"),idle=button.textContent;button.disabled=true;button.textContent=L.creating;createError.style.display="none";var data=Object.fromEntries(new FormData(form));fetch("/api/external-integrations",{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(data)}).then(function(r){return r.json().then(function(d){return{ok:r.ok,data:d}})}).then(function(x){if(!x.ok||!x.data.success)throw new Error(x.data.error||L.createFailed);token.value=x.data.data.token||"";secret.value=x.data.data.webhookSecret||"";credentials.showModal()}).catch(function(error){createError.textContent=error.message||L.createFailed;createError.style.display="block"}).finally(function(){button.disabled=false;button.textContent=idle})});credentials.querySelector("[data-role=copy-external-credentials]").addEventListener("click",function(){var button=this,value=L.copyTemplate.replace("{endpoint}",L.restEndpoint).replace("{token}",token.value).replace("{secret}",secret.value);if(window.vokoCopyText)window.vokoCopyText(value,button);else navigator.clipboard.writeText(value).then(function(){button.textContent=L.copied})});credentials.querySelector("[data-role=close-external-credentials]").addEventListener("click",function(){token.value="";secret.value="";credentials.close();location.reload()});document.addEventListener("click",function(e){var button=e.target.closest("[data-role=revoke-external-integration]");if(!button)return;pending=button.dataset.integration||"";revoke.querySelector("[data-role=revoke-external-message]").textContent=L.revokeMessage.replace("{name}",button.dataset.name||"");revoke.querySelector("[data-role=revoke-external-error]").style.display="none";revoke.showModal()});revoke.querySelector("[data-role=cancel-external-revoke]").addEventListener("click",function(){pending="";revoke.close()});revoke.querySelector("[data-role=confirm-external-revoke]").addEventListener("click",function(){var button=this;if(!pending)return;button.disabled=true;fetch("/api/external-integrations/"+encodeURIComponent(pending),{method:"DELETE",headers:{Accept:"application/json"}}).then(function(r){return r.json().then(function(d){return{ok:r.ok,data:d}})}).then(function(x){if(!x.ok||!x.data.success)throw new Error(x.data.error||L.revokeFailed);location.reload()}).catch(function(error){var box=revoke.querySelector("[data-role=revoke-external-error]");box.textContent=error.message||L.revokeFailed;box.style.display="block";button.disabled=false})})})();</script>';
  ...
  R.get('/agents/:agentId',async(req,res,next)=>{
    try{
      ...
      const requestedTab=req.query.tab==='group'?'group':req.query.tab==='owner'?'owner':req.query.tab==='a2a'?'a2a':'conv';
      ...
      const a2aLabel=L('web.agent.tab.a2a_tasks')+(a2aPrincipalTotal?' ('+a2aPrincipalTotal+')':'');
      ...
      res.send(renderPage(req,T('web.agent.title',{name:aName}),body,{nav:agentNav(agentId,agent.agentName||agent.agentId,T),msg,jsonld:{'@context':'https://schema.org',name:agent.agentName,identifier:agent.agentId},footer:renderFooter(T, req.locale)+tabScript+ownerChatLiveScript(agentId,'#agent-tabs-root')}))
    }catch(e){next(e)}
  });
  ...
  R.get('/agents/:agentId/owner-chats/:conversationId',async(req,res,next)=>{
    ...
    try{
      ...
      res.send(renderPage(req,T('web.owner_chat.title',{name:agent.agentName||agent.agentId}),body,{showTitle:false,nav:agentNav(agentId,agent.agentName||agent.agentId,T),footer:renderFooter(T,req.locale)+ownerChatLiveScript(agentId,'.owner-chat-detail',conversationId,fragmentUrl)}));
    }catch(error){next(error)}
  });
