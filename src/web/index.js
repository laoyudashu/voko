...
function copyIcon(){return '<svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" focusable="false"><rect x="9" y="9" width="10" height="10" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M15 9V7a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>'}
function accessIcon(kind){const path=kind==='visitor'?'<path d="M4 7h16v10H7l-3 3V7Z"/><path d="M8 11h8M8 14h5"/>':kind==='owner'?'<path d="M12 3 5 6v5c0 4.5 2.9 8.1 7 10 4.1-1.9 7-5.5 7-10V6l-7-3Z"/><path d="m9 12 2 2 4-4"/>':'<circle cx="6" cy="12" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="18" cy="18" r="2"/><path d="m8 11 8-4M8 13l8 4"/>';return '<svg viewBox="0 0 24 24" width="14" height="14" aria-hidden="true" focusable="false" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'+path+'</svg>'}
...
function agentWsScript(t) {
  const i18nObj = {
    ...
    owner_open: t('web.home.owner_link.open'),
    close: t('common.btn.close'),
...
var copySvg='<svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><rect x="9" y="9" width="10" height="10" rx="2" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M15 9V7a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>';
var copiedSvg='<svg viewBox="0 0 24 24" width="16" height="16" aria-hidden="true"><path d="m5 12 4 4L19 6" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
function markCopied(button){if(!button)return;clearTimeout(button._copyTimer);button.innerHTML=copiedSvg;button.classList.add("is-copied");button._copyTimer=setTimeout(function(){button.innerHTML=copySvg;button.classList.remove("is-copied")},1400)}
function ownerLinkDialog(){var dlg=document.getElementById("dlg-owner-link");if(dlg)return dlg;dlg=document.createElement("dialog");dlg.id="dlg-owner-link";dlg.style.cssText="border:none;border-radius:12px;padding:0;max-width:480px;width:calc(100% - 40px);box-shadow:0 12px 36px rgba(15,23,42,.18)";dlg.innerHTML='<div style="padding:22px 24px 18px"><h3 style="margin:0 0 7px;color:#1a1a2e"></h3><p class="meta" style="margin:0 0 12px"></p><div style="display:flex;align-items:center;gap:7px"><input id="owner-link-url" readonly style="margin:0;max-width:none;font-size:13px"><button type="button" class="home-copy-icon" data-role="copy-owner-link" aria-label=""></button></div><p id="owner-link-expiry" class="meta" style="margin:7px 0 14px"></p><form method="dialog" style="display:flex;justify-content:flex-end;gap:8px"><button class="btn-sm btn-outline" value="cancel"></button><a id="owner-link-open" class="btn-sm" target="_blank" rel="noopener noreferrer"></a></form></div>';dlg.querySelector("h3").textContent=I.owner_title;dlg.querySelector("p.meta").textContent=I.owner_warning;var copy=dlg.querySelector("[data-role=copy-owner-link]");copy.title=I.owner_copy;copy.setAttribute("aria-label",I.owner_copy);copy.innerHTML=copySvg;dlg.querySelector("button[value=cancel]").textContent=I.close;dlg.querySelector("#owner-link-open").textContent=I.owner_open;document.body.appendChild(dlg);return dlg}
function createOwnerLink(t){var aid=t.dataset.agent,original=t.textContent;t.disabled=true;t.textContent=I.gen_creating;fetch("/api/owner-link/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId:aid})}).then(function(r){return r.json().then(function(d){return{ok:r.ok,data:d}})}).then(function(x){if(!x.ok||!x.data.success)throw new Error(x.data.error||I.gen_failed);var d=x.data.data||{},dlg=ownerLinkDialog(),input=dlg.querySelector("#owner-link-url"),expiry=dlg.querySelector("#owner-link-expiry"),open=dlg.querySelector("#owner-link-open");input.value=d.ownerUrl||"";expiry.textContent=d.expiresAt?I.owner_expires+new Date(d.expiresAt).toLocaleString():"";open.href=d.ownerUrl||"#";dlg.showModal()}).catch(function(e){var m=document.getElementById("toast-msg");if(m)m.textContent=e.message||I.gen_failed;var dlg=document.getElementById("dlg-toast");if(dlg)dlg.showModal()}).finally(function(){t.disabled=false;t.textContent=original})}
(function(){
  var ws = null;
...
document.addEventListener("click",function(e){var t=e.target.closest?e.target.closest("[data-role]")||e.target:e.target;
 ...
 else if(t.matches("[data-role=gen-owner-link]")){createOwnerLink(t)}
 else if(t.matches("[data-role=copy-owner-link]")){var input=document.getElementById("owner-link-url");if(input)navigator.clipboard.writeText(input.value).then(function(){markCopied(t)}).catch(function(){})}
...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      if(opts.a2aModule){try{opts.a2aModule.withDatabase(function(a2aDb){const{A2APublicationStore}=require('../a2a');const store=new A2APublicationStore(a2aDb);const row=a2aDb.prepare("SELECT value FROM a2a_settings WHERE key='bridge_config_v1'").get();const config=row?JSON.parse(row.value):null;for(const item of pageAgents){const local=db.prepare('SELECT did FROM agents WHERE agent_id=?').get(item.agentId);const{serverAgentIdFromDid}=require('../core/agent-invitations');const publicId=serverAgentIdFromDid(local?.did)||item.agentId;const published=store.isPublicEnabled(item.agentId)&&Array.isArray(config?.registeredAgentIds)&&config.registeredAgentIds.includes(publicId);a2aPublicByAgent.set(item.agentId,{published,publicId})}})}catch{}}
      ...
      for(const a of pageAgents){
        ...
        var visitorValue='<span class="home-access-value">'+L('web.home.access.not_generated')+'</span>',visitorAction='<button class="btn btn-sm btn-outline home-access-action" data-role="gen-link" data-agent="'+esc(a.agentId)+'">'+L('common.btn.generate_link')+'</button>';
        if(db){try{var sr=db.prepare('SELECT short_link_url FROM agents WHERE agent_id=?').get(a.agentId);if(sr&&sr.short_link_url){var su=esc(sr.short_link_url);visitorValue='<a class="home-access-value" href="'+su+'" target="_blank" rel="noopener noreferrer">'+su.substring(0,35)+(su.length>35?'…':'')+'</a>';visitorAction='<button class="home-copy-icon" data-role="copy-link" data-url="'+su+'" title="'+L('web.home.access.copy_visitor')+'" aria-label="'+L('web.home.access.copy_visitor')+'">'+copyIcon()+'</button>'}}catch(ex){}}
        const a2a=a2aPublicByAgent.get(a.agentId);const a2aCard=a2a?.published?String(ENDPOINTS.api.baseUrl||'').replace(/\/+$/,'')+'/a2a/agents/'+encodeURIComponent(a2a.publicId)+'/.well-known/agent-card.json':'';
        const a2aValue='<span class="home-access-value">'+L(a2a?.published?'web.home.access.a2a_published':'web.home.access.a2a_unpublished')+'</span>';
        const a2aAction=a2a?.published?'<button class="home-copy-icon" data-role="copy-link" data-url="'+esc(a2aCard)+'" title="'+L('web.home.access.copy_a2a')+'" aria-label="'+L('web.home.access.copy_a2a')+'">'+copyIcon()+'</button>':'<a class="btn btn-sm btn-outline home-access-action home-a2a-action" href="/agents/'+esc(a.agentId)+'/caps">'+L('web.home.access.setup')+'</a>';
        var accessModeButton='<button type="button" class="btn btn-sm home-mode-toggle home-access-mode '+(a.accessMode==='private'?'home-mode-private':'home-mode-public')+'" style="margin:1px;padding:1px 6px;min-width:auto;min-height:auto;font-size:11px;line-height:1.4;border-width:2px;border-radius:6px" data-role="toggle-acc" data-agent="'+esc(a.agentId)+'" data-acc-mode="'+(a.accessMode==='private'?'private':'public')+'" title="'+esc(a.accessMode==='private'?T('common.acc.title_private'):T('common.acc.title_public'))+'">'+L(a.accessMode==='private'?'common.acc.private':'common.acc.public')+'</button>';
        var shortCell='<div class="home-access-stack"><div class="home-access-row home-access-visitor-row"><span class="home-access-label">'+accessIcon('visitor')+L('web.home.access.visitor')+'</span>'+visitorValue+visitorAction+accessModeButton+'</div><div class="home-access-row"><span class="home-access-label">'+accessIcon('owner')+L('web.home.access.owner')+'</span><span class="home-access-value">'+L('web.home.access.owner_desc')+'</span><button class="btn btn-sm btn-outline home-access-action home-owner-action" data-role="gen-owner-link" data-agent="'+esc(a.agentId)+'">'+L('common.btn.generate_link')+'</button></div><div class="home-access-row"><span class="home-access-label">'+accessIcon('a2a')+'A2A</span>'+a2aValue+a2aAction+'</div></div>';
        var actionHtml='<a href="/agents/'+esc(a.agentId)+'/edit" class="btn btn-sm btn-outline" style="margin:1px;padding:1px 6px;font-size:11px;min-height:auto">'+L('common.btn.edit')+'</a> <button type="button" class="btn btn-sm home-mode-toggle '+(a.publishStatus==='published'?'home-mode-published':'home-mode-unpublished')+'" data-role="toggle-pub" data-agent="'+esc(a.agentId)+'" data-pub-status="'+(a.publishStatus==='published'?'published':'unpublished')+'" title="'+esc(a.publishStatus==='published'?T('common.pub.title_published'):T('common.pub.title_unpublished'))+'">'+L(a.publishStatus==='published'?'common.pub.published':'common.pub.unpublished')+'</button>';
        rows.push('<tr data-agent-id="'+esc(a.agentId)+'"><td class="home-agent-name"><a href="/agents/'+esc(a.agentId)+'"'+agentNameHint+'>'+esc(agentNameDisplay)+'</a></td><td style="white-space:nowrap;font-size:14px;text-align:center">'+esc(bt)+'</td><td data-role="connection-status" style="white-space:nowrap;font-size:14px;text-align:center">'+connStatus+'</td><td data-role="message-mode" data-message-mode-detected="'+(messageModeDetected?'true':'false')+'" style="white-space:nowrap;font-size:14px;text-align:center">'+esc(messageMode)+'</td><td class="home-agent-short" style="font-size:13px">'+shortCell+'</td><td class="home-agent-actions" style="white-space:nowrap;font-size:13px;text-align:center">'+actionHtml+'</td></tr>');        jd.push({name:a.agentName,identifier:a.agentId})
      }
  ...
  R.get('/agents/:agentId/c/:channelId',async(req,res,next)=>{
    try{
      ...
      const newestMessageConversation=(md.messages||[]).map(m=>{
        const messageId=m.messageId||m.id;
        const route=messageId?messageRoutes.getByMessage(String(messageId),agentId):null;
      ...
      if(webRoutingEnabled('web_private_conversations_v1')){
        const cards=webConversations.map((c,i)=>'<a class="conversation-tab-card'+(selectedConversation?.id===c.id&&!showLegacyHistory?' active':'')+'" role="tab" aria-selected="'+(selectedConversation?.id===c.id&&!showLegacyHistory)+'" href="/agents/'+esc(agentId)+'/c/'+esc(channelId)+'?conversationId='+encodeURIComponent(c.id)+'">'+esc(T('web.conversation.tab.label',{index:i+1}))+'</a>').join('');
        const history=showLegacyHistory||(!webConversations.length&&msgs.length)?'<a class="conversation-tab-card'+(showLegacyHistory?' active':'')+'" role="tab" href="/agents/'+esc(agentId)+'/c/'+esc(channelId)+'?history=1">'+L('web.conversation.tab.history')+'</a>':'';
        const newConversationLabel=pendingConversation?L('web.conversation.action.continue_pending'):L('web.conversation.action.new');
        conversationTabs='<style>'+CONVERSATION_TAB_CSS+'</style><div class="conversation-tab-shell"><button type="button" class="conversation-tab-arrow" id="conversation-tabs-prev" aria-label="'+esc(T('web.payments.prev_page'))+'">‹</button><div class="conversation-tab-rail" id="conversation-tab-rail" role="tablist">'+cards+history+'<button type="button" id="new-conversation" data-pending-conversation="'+esc(pendingConversation?.id||'')+'" class="conversation-tab-card conversation-tab-new">＋ '+newConversationLabel+'</button></div><button type="button" class="conversation-tab-arrow" id="conversation-tabs-next" aria-label="'+esc(T('web.payments.next_page'))+'">›</button></div>';
      }
      ...
      const conversationControlScript='<script>(function(){var aid='+jsonForInlineScript(agentId)+',cid='+jsonForInlineScript(channelId)+',selected='+jsonForInlineScript(selectedConversation?.id||'')+',createError='+jsonForInlineScript(T('web.conversation.error.create'))+';function conversationUrl(id){return "/agents/"+encodeURIComponent(aid)+"/c/"+encodeURIComponent(cid)+"?conversationId="+encodeURIComponent(id)}function post(url,body){return fetch(url,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(body)}).then(function(r){return r.json()})}var create=document.getElementById("new-conversation");if(create)create.onclick=function(){var pending=create.getAttribute("data-pending-conversation");if(pending){location.href=conversationUrl(pending);return}create.disabled=true;post("/api/routing-conversations/create",{agentId:aid,channelId:cid,channelType:1,parentConversationId:selected||null}).then(function(j){if(j.success)location.href=conversationUrl(j.conversation.id);else throw new Error(j.error||createError)}).catch(function(e){create.disabled=false;alert(e&&e.message||createError)})};var rail=document.getElementById("conversation-tab-rail"),prev=document.getElementById("conversation-tabs-prev"),next=document.getElementById("conversation-tabs-next");function state(){if(!rail)return;var max=Math.max(0,rail.scrollWidth-rail.clientWidth);if(prev)prev.disabled=rail.scrollLeft<2;if(next)next.disabled=rail.scrollLeft>=max-2}function move(dir){if(rail)rail.scrollBy({left:dir*Math.max(180,rail.clientWidth*.7),behavior:"smooth"})}if(prev)prev.onclick=function(){move(-1)};if(next)next.onclick=function(){move(1)};if(rail){rail.addEventListener("scroll",state,{passive:true});window.addEventListener("resize",state);var active=rail.querySelector(".conversation-tab-card.active");if(active)active.scrollIntoView({inline:"center",block:"nearest"});state()}})();</script>';
    ...
    try{
      ...
      res.send(renderAgentFormPage(T('web.agent.edit.title'),agentId,aname,
        ...
        +'var w=document.getElementById("bt-wrapper"),tr=document.getElementById("bt-trigger"),dd=document.getElementById("bt-dropdown"),bs=document.getElementById("bt-search"),bt=document.getElementById("bt"),tx=document.getElementById("bt-text"),oc=document.getElementById("bt-options"),ifield=document.getElementById("bt-instance-field"),isel=document.getElementById("bt-instance"),ivalue=document.getElementById("bt-instance-value");'
         +'var all=[],loaded=false,loading=false,backendTypes=[],INSTANCE_TYPES={openclaw:true,hermes:true,zeroclaw:true},TXT_NO_MATCH='+jsonForInlineScript(T('web.agent.edit.no_match'))+',TXT_LOADING='+jsonForInlineScript(T('web.agent.edit.types_loading'))+',TXT_FAILED='+jsonForInlineScript(T('web.agent.edit.types_load_failed'))+',TXT_LOCAL='+jsonForInlineScript(T('register.flow.provider.local'))+',TXT_MORE='+jsonForInlineScript(T('register.flow.provider.more'))+',TXT_INSTANCE_HINT='+jsonForInlineScript(T('web.agent.edit.instance_hint'))+',TXT_INSTANCE_LOADING='+jsonForInlineScript(T('web.agent.edit.instances_loading'))+',TXT_INSTANCE_NONE='+jsonForInlineScript(T('web.agent.edit.instances_none'))+',TXT_INSTANCE_STALE='+jsonForInlineScript(T('web.agent.edit.instance_stale'))+',AGENT_ID='+jsonForInlineScript(agentId)+',INITIAL_TYPE='+jsonForInlineScript(p.backendType||'')+',INITIAL_INSTANCE='+jsonForInlineScript(p.backendInstanceId||'')+';'
...
try{const r=await handlers.list_access_lists({agentId,listType:'whitelist',limit,offset,keyword});const es=r.data||r.entries||r.accessList||[],total=r.total||es.length;totalPages=Math.ceil(total/limit);let wlNickMap={};if(es.length){try{const vids=es.map(e=>e.visitor_id||e.visitorId||'').filter(Boolean);if(vids.length){const rows=db.prepare('SELECT uid, nickname FROM user_cache WHERE uid IN ('+vids.map(()=>'?').join(',')+')').all(...vids);rows.forEach(r=>{wlNickMap[r.uid]=r.nickname||'';});}}catch(_){}const wlName=(vid)=>{const n=wlNickMap[vid];return n?esc(n)+' ('+esc(vid)+')':esc(vid);};const wlSource=(e)=>e.source==='same_owner_default'?(e.auto_trust_disabled?'已关闭自动信任':'同一主人自动信任'):(e.source==='outbound_contact'?'主动联系自动加入':'手动添加');listHtml='<div class="table-wrap"><table><thead><tr><th>'+L('web.agent.whitelist.col.visitor')+'</th><th>来源</th><th>'+L('web.agent.whitelist.col.reason')+'</th><th style="text-align:center">'+L('web.agent.whitelist.col.action')+'</th></tr></thead><tbody>'+es.map(e=>'<tr><td>'+wlName(e.visitor_id||e.visitorId||'')+'</td><td>'+esc(wlSource(e))+'</td><td>'+esc(e.reason||'-')+'</td><td style="text-align:center"><form method="POST" action="/agents/'+esc(agentId)+'" data-voko-access-list style="display:inline"><input type="hidden" name="_action" value="remove_whitelist"><input type="hidden" name="visitorId" value="'+esc(e.visitor_id||e.visitorId||'')+'"><button type="submit" class="btn-xs btn-outline" style="margin:0;padding:2px 8px;font-size:11px;min-height:auto">'+L('common.btn.remove')+'</button></form></td></tr>').join('\n')+'</tbody></table></div>'}}catch{}
  ...
  R.get('/agents/:agentId/caps',async(req,res,next)=>{
    try{
      ...
      const publicationRow=db.prepare('SELECT did,publish_status FROM agents WHERE agent_id=?').get(agentId)||{};
      const vokoPublished=publicationRow.publish_status==='published';let a2aPublic=true,a2aPublished=false;
      try{opts.a2aModule.withDatabase(function(a2aDb){const{A2APublicationStore}=require('../a2a');const stored=new A2APublicationStore(a2aDb).getPublicEnabled(agentId);a2aPublic=stored===null?true:stored;const row=a2aDb.prepare("SELECT value FROM a2a_settings WHERE key='bridge_config_v1'").get();const config=row?JSON.parse(row.value):null;const{serverAgentIdFromDid}=require('../core/agent-invitations');const publicId=serverAgentIdFromDid(publicationRow.did)||agentId;a2aPublished=stored===true&&Array.isArray(config?.registeredAgentIds)&&config.registeredAgentIds.includes(publicId)})}catch{}
      ...
      const{serverAgentIdFromDid}=require('../core/agent-invitations');const publicAgentId=serverAgentIdFromDid(publicationRow.did)||agentId;
      const a2aBase=String(require('../endpoints.json').api.baseUrl||'').replace(/\/+$/,'');const cardUrl=a2aBase+'/a2a/agents/'+encodeURIComponent(publicAgentId)+'/.well-known/agent-card.json';const callUrl=a2aBase+'/a2a/v1/agents/'+encodeURIComponent(publicAgentId);
      const a2aClientScript='<script>(function(){var copyText='+JSON.stringify(T('common.btn.copy')).replace(/</g,'\\u003c')+',copiedText='+JSON.stringify(T('common.home.copied')).replace(/</g,'\\u003c')+';document.querySelectorAll("[data-a2a-copy]").forEach(function(button){button.addEventListener("click",async function(){var value=button.getAttribute("data-a2a-copy"),ok=false;try{await navigator.clipboard.writeText(value);ok=true}catch(_){var area=document.createElement("textarea");area.value=value;area.style.position="fixed";area.style.opacity="0";document.body.appendChild(area);area.select();try{ok=document.execCommand("copy")}catch(_){}area.remove()}if(ok){button.textContent=copiedText;setTimeout(function(){button.textContent=copyText},1600)}})});var verify=document.getElementById("verify-a2a-card"),feedback=document.getElementById("a2a-public-feedback");if(verify)verify.addEventListener("click",async function(){verify.disabled=true;verify.textContent='+JSON.stringify(T('web.agent.caps.verifying')).replace(/</g,'\\u003c')+';feedback.textContent="";try{var response=await fetch("/agents/'+encodeURIComponent(agentId)+'/caps/verify-a2a-card",{method:"POST"}),result=await response.json();feedback.textContent=result.message||result.error||"";feedback.style.color=result.success?"#26834a":"#b3261e"}catch(_){feedback.textContent='+JSON.stringify(T('web.agent.caps.verify_failed')).replace(/</g,'\\u003c')+';feedback.style.color="#b3261e"}finally{verify.disabled=false;verify.textContent='+JSON.stringify(T('web.agent.caps.verify_card')).replace(/</g,'\\u003c')+'}})})();</script>';
      const capsValidationScript='<script>(function(){var original='+JSON.stringify({abilities,a2aPublic}).replace(/</g,'\\u003c')+',noChanges='+JSON.stringify(T('web.agent.caps.no_changes')).replace(/</g,'\\u003c')+',invalid='+JSON.stringify(T('web.agent.caps.invalid_local')).replace(/</g,'\\u003c')+';function canonical(items){return items.map(function(item){return{id:String(item.id||""),name:String(item.name||"").trim(),description:String(item.description||"").trim(),tags:Array.isArray(item.tags)?item.tags.map(String).map(function(v){return v.trim()}).filter(Boolean):[],examples:Array.isArray(item.examples)?item.examples.map(String).map(function(v){return v.trim()}).filter(Boolean):[],fields:Array.isArray(item.fields)?item.fields.map(function(field){return{name:String(field.name||"").trim(),type:String(field.type||"string")}}):[]}})}document.addEventListener("DOMContentLoaded",function(){var form=document.getElementById("caps-form");if(!form)return;var submit=form.querySelector("button[type=submit]"),status=document.createElement("div");status.id="caps-local-status";status.setAttribute("role","status");status.setAttribute("aria-live","polite");status.style.cssText="display:none;margin:8px 0";form.insertBefore(status,submit);form.addEventListener("submit",function(event){var raw;try{raw=JSON.parse(document.getElementById("caps-json").value||"[]")}catch(_){raw=null}var valid=Array.isArray(raw)&&raw.every(function(item){return item&&typeof item==="object"&&String(item.name||"").trim()&&String(item.description||"").trim()&&Array.isArray(item.tags)&&item.tags.length>0&&Array.isArray(item.fields)&&item.fields.every(function(field){return field&&String(field.name||"").trim()})});if(!valid){event.preventDefault();status.textContent=invalid;status.className="error";status.style.display="block";return}var current={abilities:canonical(raw),a2aPublic:!!form.querySelector("[name=publishA2A]").checked},before={abilities:canonical(original.abilities||[]),a2aPublic:!!original.a2aPublic};if(JSON.stringify(current)===JSON.stringify(before)){event.preventDefault();status.textContent=noChanges;status.className="meta";status.style.display="block"}else status.style.display="none"})})})();</script>';
      const a2aInfo=(a2aPublished?'<details class="a2a-public-info"><summary>'+L('web.agent.caps.public_info')+'</summary><div class="a2a-public-body">'+[['did',T('web.agent.caps.public_did'),publicationRow.did],['card',T('web.agent.caps.card_url'),cardUrl],['endpoint',T('web.agent.caps.endpoint_url'),callUrl]].map(function(item){return '<div class="a2a-public-row"><strong>'+esc(item[1])+'</strong><code title="'+esc(item[2])+'">'+esc(item[2])+'</code><span class="a2a-public-actions"><button type="button" class="btn-sm btn-outline" data-a2a-copy="'+esc(item[2])+'">'+L('common.btn.copy')+'</button>'+(item[0]==='card'?'<a class="btn-sm btn-outline" href="'+esc(item[2])+'" target="_blank" rel="noopener">'+L('web.agent.caps.open_card')+'</a><button type="button" class="btn-sm btn-outline" id="verify-a2a-card">'+L('web.agent.caps.verify_card')+'</button>':'')+'</span></div>'}).join('')+'<div id="a2a-public-feedback" class="meta" role="status" aria-live="polite"></div></div></details>'+a2aClientScript:'<p class="meta" style="flex-basis:100%;margin:4px 0 0 28px">'+L('web.agent.caps.public_unpublished_hint')+'</p>')+capsValidationScript;
      ...
      try{
        switch(a){
          ...
          case'declare_caps':{
            ...
            opts.a2aModule?.withDatabase(function(a2aDb){const{A2APublicationStore}=require('../a2a');new A2APublicationStore(a2aDb).setPublicEnabled(agentId,req.body.publishA2A==='true')});
            if(req.body.publishA2A==='true'&&!opts.syncA2ARegistration)return res.redirect(actionResultLocation(capsPath,'warn',req.t('web.agent.caps.voko_only')));
            if(req.body.publishA2A==='true'){try{const sync=await opts.syncA2ARegistration();
            ...
            }catch(error){console.error('[A2A] 能力发布状态同步失败:',error.message);return res.redirect(actionResultLocation(capsPath,'warn',req.t('web.agent.caps.voko_only')))}}
            return res.redirect(actionResultLocation(capsPath,'ok',req.t(req.body.publishA2A==='true'?'web.agent.caps.declared_a2a':'web.agent.caps.declared_voko')))
          }
...
+'- Operation results: ActionStatusType JSON-LD\n');});
  ...
  R.post('/api/owner-link/create', async (req, res) => {
    try {
      ...
      if(!localAgentId)return res.status(400).json({success:false,error:req.t('web.home.owner_link.agent_required')});
      ...
      const ownerEmail=String(row.owner_email||currentOwnerEmail()).trim().toLowerCase();
      const userAccessToken=getUserAccessToken(db,ownerEmail);
      ...
      const response=await fetch(base+'/api/owner-links/v1',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+userAccessToken},body:JSON.stringify({agentId:serverAgentId}),signal:AbortSignal.timeout(10000)});
