...
function renderMessageModeMenu(details,status){var menu=details.querySelector("[data-role=message-mode-menu]"),preferred=String(status.temporaryPreferredMode||""),preferredProvider=String(status.temporaryPreferredProvider||"");if(!menu)return;menu.innerHTML=(status.methods||[]).map(function(method){var manuallySelected=!!preferred&&method.mode===preferred&&String(method.provider||"")===preferredProvider,activeSelected=!preferred&&method.mode===status.activeAutomaticMode,selected=manuallySelected||activeSelected,enabled=method.available===true,workbuddy=status.backendType==='workbuddy'&&method.mode==='http'&&method.provider==='workbuddy-http',state=manuallySelected?I.message_mode_temporary:(activeSelected?I.message_mode_active:(enabled?(method.configured?I.message_mode_available:I.message_mode_enable):I.message_mode_unavailable)),option='<button type="button" class="home-message-mode-option'+(selected?' is-selected':'')+'" data-role="select-message-mode" data-mode="'+String(method.mode).replace(/"/g,'&quot;')+'" data-provider="'+String(method.provider||'').replace(/"/g,'&quot;')+'" '+(enabled?'':'disabled')+'><span class="home-message-mode-dot '+(enabled?'is-available':'is-unavailable')+'"></span><span class="home-message-mode-name">'+messageModeLabel(method.mode)+'</span><span class="home-message-mode-state">'+state+'</span></button>';if(workbuddy&&!enabled)option+='<button type="button" class="home-message-mode-option" data-role="setup-workbuddy"><span class="home-message-mode-name">'+I.message_mode_setup+'</span><span class="home-message-mode-state">›</span></button>';return option}).join('');requestAnimationFrame(function(){positionMessageModeMenu(details)})}
function renderWorkBuddySetup(details){var menu=details.querySelector("[data-role=message-mode-menu]");if(!menu)return;menu.innerHTML='<div class="home-message-mode-setup"><strong>'+I.message_mode_setup_title+'</strong><p>'+I.message_mode_setup_help+'</p><code>npm install -g @tencent-ai/codebuddy-code</code><button type="button" class="btn-sm btn-outline" data-role="copy-workbuddy-command" data-command="npm install -g @tencent-ai/codebuddy-code">'+I.message_mode_copy_install+'</button><code>codebuddy /login</code><button type="button" class="btn-sm btn-outline" data-role="copy-workbuddy-command" data-command="codebuddy /login">'+I.message_mode_copy_login+'</button><button type="button" class="btn-sm" data-role="recheck-workbuddy">'+I.message_mode_recheck+'</button></div>';positionMessageModeMenu(details)}
...
document.addEventListener("click",function(e){var setup=e.target.closest&&e.target.closest("[data-role=setup-workbuddy]");if(setup){renderWorkBuddySetup(setup.closest("details[data-role=message-mode-picker]"));return}var copy=e.target.closest&&e.target.closest("[data-role=copy-workbuddy-command]");if(copy){navigator.clipboard&&navigator.clipboard.writeText(copy.dataset.command||"");return}var recheck=e.target.closest&&e.target.closest("[data-role=recheck-workbuddy]");if(recheck){var details=recheck.closest("details[data-role=message-mode-picker]");if(!confirm(I.message_mode_loopback_confirm))return;recheck.disabled=true;recheck.textContent=I.message_mode_checking;fetch("/api/agents/"+encodeURIComponent(details.dataset.agent||"")+"/delivery-channels/verify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({providerId:"workbuddy-http"})}).then(function(r){return r.json().then(function(d){return{ok:r.ok,data:d}})}).then(function(result){if(!result.ok||!result.data.success)throw new Error(result.data.error||I.message_mode_switch_failed);renderMessageModeMenu(details,result.data.deliveryStatus||{})}).catch(function(error){var menu=details.querySelector("[data-role=message-mode-menu]");if(menu)menu.innerHTML='<span class="home-message-mode-loading is-error">'+(error.message||I.message_mode_switch_failed)+'</span>'})}});
