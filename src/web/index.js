...
function renderMessageModeMenu(details,status){var menu=details.querySelector("[data-role=message-mode-menu]"),preferred=String(status.temporaryPreferredMode||""),preferredProvider=String(status.temporaryPreferredProvider||"");if(!menu)return;menu.innerHTML=(status.methods||[]).map(function(method){var manuallySelected=!!preferred&&method.mode===preferred&&String(method.provider||"")===preferredProvider,activeSelected=!preferred&&method.mode===status.activeAutomaticMode,selected=manuallySelected||activeSelected,enabled=method.available===true,workbuddy=status.backendType==='workbuddy'&&method.mode==='http'&&method.provider==='workbuddy-http',qwen=status.backendType==='qwen-office'&&method.mode==='cli'&&method.provider==='qwen-office-cli',state=manuallySelected?I.message_mode_temporary:(activeSelected?I.message_mode_active:(enabled?(method.configured?I.message_mode_available:I.message_mode_enable):I.message_mode_unavailable)),option='<button type="button" class="home-message-mode-option'+(selected?' is-selected':'')+'" data-role="select-message-mode" data-mode="'+String(method.mode).replace(/"/g,'&quot;')+'" data-provider="'+String(method.provider||'').replace(/"/g,'&quot;')+'" '+(enabled?'':'disabled')+'><span class="home-message-mode-dot '+(enabled?'is-available':'is-unavailable')+'"></span><span class="home-message-mode-name">'+messageModeLabel(method.mode)+'</span><span class="home-message-mode-state">'+state+'</span></button>';if(workbuddy&&!enabled)option+='<button type="button" class="home-message-mode-option home-message-mode-setup-option" style="margin-left:17px;width:calc(100% - 17px);padding-left:8px;border-left:2px solid #dbe7f7" data-role="setup-workbuddy"><span aria-hidden="true"></span><span class="home-message-mode-name">'+I.message_mode_setup+'</span><span class="home-message-mode-state" style="white-space:nowrap">'+I.message_mode_setup_summary+' ›</span></button>';if(qwen&&!enabled)option+='<button type="button" class="home-message-mode-option home-message-mode-setup-option" style="margin-left:17px;width:calc(100% - 17px);padding-left:8px;border-left:2px solid #dbe7f7" data-role="setup-qwen" data-command="'+String(method.setupCommand||'qoderclicn login').replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'"><span aria-hidden="true"></span><span class="home-message-mode-name">'+I.message_mode_setup+'</span><span class="home-message-mode-state" style="white-space:nowrap">'+I.message_mode_setup_summary+' ›</span></button>';return option}).join('');requestAnimationFrame(function(){positionMessageModeMenu(details)})}
function renderWorkBuddySetup(details){var dialog=document.getElementById("dlg-workbuddy-setup");if(!dialog){dialog=document.createElement("dialog");dialog.id="dlg-workbuddy-setup";dialog.className="voko-message-dialog";dialog.style.cssText="width:min(440px,calc(100vw - 24px));max-height:calc(100vh - 24px);padding:0;border:0;border-radius:14px;box-shadow:0 22px 70px rgba(15,23,42,.28);overflow:hidden";dialog.innerHTML='<div class="home-message-mode-setup" style="display:grid;gap:12px;max-height:calc(100vh - 24px);overflow:auto;padding:20px;color:#344054;font-size:13px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><strong style="font-size:16px">'+I.message_mode_setup_title+'</strong><button type="button" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;min-width:32px;min-height:32px;margin:0;padding:0;border:0;border-radius:50%;background:transparent;color:#667085;font-size:24px;line-height:1" data-role="close-workbuddy-setup" title="'+I.message_mode_close+'" aria-label="'+I.message_mode_close+'">×</button></div><p style="margin:0;color:#667085;line-height:1.55">'+I.message_mode_setup_help+'</p><div class="home-message-mode-setup-step" style="display:grid;grid-template-columns:minmax(0,1fr) 28px;gap:5px 7px;padding-left:12px;border-left:3px solid #dbe7f7"><strong style="grid-column:1/-1">'+I.message_mode_install_title+'</strong><p style="grid-column:1/-1;margin:0;color:#667085;line-height:1.5">'+I.message_mode_install_help+'</p><code style="display:block;overflow-wrap:anywhere;padding:7px;border-radius:6px;background:#f2f4f7">npm install -g @tencent-ai/codebuddy-code</code><button type="button" class="voko-copy-button" data-role="copy-workbuddy-command" data-voko-copy-value="npm install -g @tencent-ai/codebuddy-code" title="'+I.message_mode_copy_install+'" aria-label="'+I.message_mode_copy_install+'">'+workBuddyCopySvg+'</button></div><div class="home-message-mode-setup-step" style="display:grid;grid-template-columns:minmax(0,1fr) 28px;gap:5px 7px;padding-left:12px;border-left:3px solid #dbe7f7"><strong style="grid-column:1/-1">'+I.message_mode_login_title+'</strong><p style="grid-column:1/-1;margin:0;color:#667085;line-height:1.5">'+I.message_mode_login_help+'</p><code style="display:block;overflow-wrap:anywhere;padding:7px;border-radius:6px;background:#f2f4f7">codebuddy /login</code><button type="button" class="voko-copy-button" data-role="copy-workbuddy-command" data-voko-copy-value="codebuddy /login" title="'+I.message_mode_copy_login+'" aria-label="'+I.message_mode_copy_login+'">'+workBuddyCopySvg+'</button></div><button type="button" class="btn-sm" style="margin:0" data-role="recheck-workbuddy">'+I.message_mode_recheck+'</button></div>';document.body.appendChild(dialog)}dialog._messageModeDetails=details;details.open=false;dialog.showModal()}
function renderQwenSetup(details,command){var dialog=document.getElementById("dlg-qwen-setup");if(!dialog){dialog=document.createElement("dialog");dialog.id="dlg-qwen-setup";dialog.className="voko-message-dialog";dialog.style.cssText="width:min(440px,calc(100vw - 24px));max-height:calc(100vh - 24px);padding:0;border:0;border-radius:14px;box-shadow:0 22px 70px rgba(15,23,42,.28);overflow:hidden";document.body.appendChild(dialog)}dialog.innerHTML='<div class="home-message-mode-setup" style="display:grid;gap:12px;max-height:calc(100vh - 24px);overflow:auto;padding:20px;color:#344054;font-size:13px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><strong style="font-size:16px">'+I.qwen_setup_title+'</strong><button type="button" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;margin:0;padding:0;border:0;border-radius:50%;background:transparent;color:#667085;font-size:24px;line-height:1" data-role="close-provider-setup" title="'+I.message_mode_close+'" aria-label="'+I.message_mode_close+'">×</button></div><p style="margin:0;color:#667085;line-height:1.55">'+I.qwen_setup_help+'</p><div class="home-message-mode-setup-step" style="display:grid;grid-template-columns:minmax(0,1fr) 28px;gap:5px 7px;padding-left:12px;border-left:3px solid #dbe7f7"><strong style="grid-column:1/-1">'+I.qwen_login_title+'</strong><p style="grid-column:1/-1;margin:0;color:#667085;line-height:1.5">'+I.qwen_login_help+'</p><code style="display:block;overflow-wrap:anywhere;padding:7px;border-radius:6px;background:#f2f4f7">'+String(command||'qoderclicn login').replace(/&/g,'&amp;').replace(/</g,'&lt;')+'</code><button type="button" class="voko-copy-button" data-voko-copy-value="'+String(command||'qoderclicn login').replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'" title="'+I.message_mode_copy_login+'" aria-label="'+I.message_mode_copy_login+'">'+workBuddyCopySvg+'</button></div><button type="button" class="btn-sm" style="margin:0" data-role="recheck-qwen">'+I.message_mode_recheck+'</button></div>';dialog._messageModeDetails=details;details.open=false;dialog.showModal()}
...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      // 信息栏
      const messageModeStyle='<style>.home-message-mode-picker{position:relative;display:inline-block}.home-message-mode-picker summary{list-style:none;cursor:pointer;color:#175cd3;border-radius:6px;padding:2px 18px 2px 6px;position:relative}.home-message-mode-picker summary::-webkit-details-marker{display:none}.home-message-mode-picker summary:after{content:"▾";position:absolute;right:4px;color:#667085}.home-message-mode-picker[open] summary{background:#eff6ff}.home-message-mode-picker.is-agent-offline summary{color:#b0b5bd;cursor:not-allowed;pointer-events:none}.home-message-mode-picker.is-agent-offline summary:after{color:#c8ccd2}.home-message-mode-menu{position:absolute;z-index:30;top:calc(100% + 5px);left:50%;transform:translateX(-50%);width:240px;padding:6px;background:#fff;border:1px solid #d0d5dd;border-radius:10px;box-shadow:0 10px 28px rgba(15,23,42,.16);text-align:left}.home-message-mode-picker.is-dropup .home-message-mode-menu{top:auto;bottom:calc(100% + 5px)}.home-message-mode-option{display:grid;grid-template-columns:10px minmax(0,1fr) auto;align-items:center;gap:7px;width:100%;margin:0;padding:8px;border:0;border-radius:7px;background:transparent;color:#344054;text-align:left;font-size:13px;cursor:pointer}.home-message-mode-option:hover:not(:disabled){background:#f2f7ff}.home-message-mode-option:disabled{cursor:not-allowed;color:#98a2b3}.home-message-mode-option.is-selected{background:#edf9f1}.home-message-mode-dot{width:8px;height:8px;border-radius:50%}.home-message-mode-dot.is-available{background:#12b76a}.home-message-mode-dot.is-unavailable{background:#98a2b3}.home-message-mode-name{font-weight:600}.home-message-mode-state{font-size:11px;color:#667085}.home-message-mode-loading{display:block;padding:9px;color:#667085;font-size:12px;text-align:center}.home-message-mode-loading.is-error{color:#b42318}.home-message-mode-setup{display:grid;gap:7px;padding:8px;color:#344054;font-size:12px}.home-message-mode-setup p{margin:0;color:#667085;line-height:1.45}.home-message-mode-setup code{display:block;overflow-wrap:anywhere;padding:6px;border-radius:6px;background:#f2f4f7;color:#101828}.home-message-mode-setup button{margin:0;width:100%;min-height:30px}</style>';
