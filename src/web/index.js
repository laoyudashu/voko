...
loadOwnerDevices();connectOwnerDeviceEvents();window.addEventListener("focus",loadOwnerDevices);document.addEventListener("visibilitychange",function(){if(!document.hidden)loadOwnerDevices()});
(function(){
  var ws = null;
...
document.addEventListener("click",function(e){var t=e.target.closest?e.target.closest("[data-role]")||e.target:e.target;
 ...
 else if(t.matches("[data-role=owner-devices]")){openOwnerDevices(t)}
 else if(t.matches("[data-role=disconnect-owner-device]")){disconnectOwnerDevice(t)}
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        var shortCell='<div class="home-access-stack"><div class="home-access-row home-access-visitor-row"><span class="home-access-label">'+accessIcon('visitor')+L('web.home.access.visitor')+'</span>'+visitorValue+visitorAction+accessModeButton+'</div><div class="home-access-row" data-owner-agent="'+esc(a.agentId)+'"><span class="home-access-label">'+accessIcon('owner')+L('web.home.access.owner')+'</span><button type="button" class="home-access-value home-owner-devices" data-role="owner-devices" data-agent="'+esc(a.agentId)+'" data-agent-name="'+esc(a.agentName||a.agentId)+'" style="appearance:none;border:0;background:none;margin:0;padding:0;min-height:auto!important;font:inherit;line-height:1.4;color:#667085;cursor:pointer;text-align:left;align-self:center">'+L('web.home.access.owner_desc')+'</button><span data-owner-link-actions><button class="btn btn-sm btn-outline home-access-action home-owner-action" data-role="gen-owner-link" data-agent="'+esc(a.agentId)+'" data-agent-name="'+esc(a.agentName||a.agentId)+'">'+L('web.home.owner_link.verify_device')+'</button></span></div><div class="home-access-row"><span class="home-access-label">'+accessIcon('a2a')+'A2A</span>'+a2aValue+a2aAction+'</div></div>';
...
+'- Operation results: ActionStatusType JSON-LD\n');});
  ...
  R.get('/api/owner-link/devices', async (req, res) => {
    try {
      ...
      const remoteToLocal=new Map(db.prepare('SELECT agent_id,did FROM agents').all().map(row=>[serverAgentIdFromDid(row.did),row.agent_id]));
      const devices=(result.data?.devices||[]).map(device=>({...device,agentId:remoteToLocal.get(device.agentId)||device.agentId}));
