...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const supportedHeader='<div class="info-bar"><span><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</span><span><strong>'+(zh?'安全适配器':'Security adapter')+':</strong> <code>'+esc(data.transportId||'-')+'</code></span></div>';
      const body=(hasVisibleControls?supportedHeader+scope:'')+form+'<p><a href="/agents/'+esc(agentId)+'">← '+(zh?'返回智能体':'Back to Agent')+'</a></p>';
