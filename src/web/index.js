...
function initialLatestScrollScript(selector,{page=false}={}){
  return '<script>(function(){var target=document.querySelector('+jsonForInlineScript(selector)+');if(!target)return;var following=true,observer=null,timer=null;function latest(){if(!following)return;if('+String(page)+')window.scrollTo(0,document.documentElement.scrollHeight);else target.scrollTop=target.scrollHeight}function stop(){following=false;if(observer)observer.disconnect();if(timer)clearTimeout(timer)}var eventsTarget='+String(page)+'?window:target;eventsTarget.addEventListener("wheel",stop,{passive:true,once:true});eventsTarget.addEventListener("touchstart",stop,{passive:true,once:true});eventsTarget.addEventListener("pointerdown",stop,{passive:true,once:true});requestAnimationFrame(function(){latest();requestAnimationFrame(latest)});window.addEventListener("load",latest,{once:true});if(window.ResizeObserver){observer=new ResizeObserver(latest);observer.observe(target)}timer=setTimeout(stop,2000)})();</script>';
}
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        const a2aAction=!a2a?.declared?'<a class="voko-copy-button home-a2a-declare-link" href="/agents/'+encodeURIComponent(a.agentId)+'/caps" title="'+L('web.home.access.declare_a2a')+'" aria-label="'+L('web.home.access.declare_a2a')+'">'+jumpIcon()+'</a>':a2a?.published?copyButton({esc,label:L('web.home.access.copy_a2a'),attrs:'data-voko-copy-value="'+esc(a2aCard)+'"'}):copyButton({esc,label:L('web.home.access.copy_a2a'),attrs:'disabled'});
