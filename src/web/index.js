...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const rows=visibleControls.filter(item=>!(promptControl&&item.id==='additionalPrompt')).map(item=>{
        let field='<span class="badge '+(item.enforcement==='unsupported'?'badge-offline':'badge-online')+'">'+esc(statusLabel(item))+'</span>';
        if(item.editable&&item.kind==='text'){field='<div><textarea name="'+esc(item.id)+'" data-control="'+esc(item.id)+'" data-risk="medium" maxlength="'+Number(item.maxLength||2000)+'" rows="7" style="margin:0;max-width:none">'+esc(data.config[item.id]||'')+'</textarea><div style="margin-top:7px"><span class="badge badge-pending" data-risk-indicator>'+(zh?'中等风险':'Medium risk')+'</span> <span class="meta" data-char-count></span></div></div>'}
      ...
      const saveBlocked=['changed_unverified','failed'].includes(observedState)||data.capabilityProbing===true||selectedTransport?.securitySelectable===false;
