...
function renderMessageModeMenu(details,status){var menu=details.querySelector("[data-role=message-mode-menu]"),preferred=String(status.temporaryPreferredMode||""),preferredProvider=String(status.temporaryPreferredProvider||"");if(!menu)return;var html=(status.methods||[]).map(function(method){var manuallySelected=!!preferred&&method.mode===preferred&&String(method.provider||"")===preferredProvider,activeSelected=!preferred&&method.mode===status.activeAutomaticMode,selected=manuallySelected||activeSelected,enabled=method.available===true,state=manuallySelected?I.message_mode_temporary:(activeSelected?I.message_mode_active:(enabled?(method.configured?I.message_mode_available:I.message_mode_enable):I.message_mode_unavailable));return '<button type="button" class="home-message-mode-option'+(selected?' is-selected':'')+'" data-role="select-message-mode" data-mode="'+String(method.mode).replace(/"/g,'&quot;')+'" data-provider="'+String(method.provider||'').replace(/"/g,'&quot;')+'" '+(enabled?'':'disabled')+'><span class="home-message-mode-dot '+(enabled?'is-available':'is-unavailable')+'"></span><span class="home-message-mode-name">'+messageModeLabel(method.mode)+'</span><span class="home-message-mode-state">'+state+'</span></button>'}).join('');var setup=status.backendType==='workbuddy'?{role:'setup-workbuddy',notice:I.message_mode_workbuddy_notice}:status.backendType==='qwen-office'?{role:'setup-qwen',notice:I.message_mode_qwen_notice}:status.backendType==='dumate'?{role:'setup-dumate',notice:I.message_mode_dumate_notice}:null;if(setup)html+='<div style="margin:6px 8px 2px;padding:9px 10px;border:1px solid #f0cf78;border-radius:8px;background:#fff8df;color:#704b00;font-size:12px;line-height:1.45"><div>'+setup.notice+'</div><button type="button" class="btn-sm btn-outline" style="margin:7px 0 0;padding:4px 8px" data-role="'+setup.role+'">'+I.message_mode_setup+'</button></div>';menu.innerHTML=html;requestAnimationFrame(function(){positionMessageModeMenu(details)})}
function providerSetupDialog(details,id,title,help,actions){var dialog=document.getElementById(id);if(!dialog){dialog=document.createElement("dialog");dialog.id=id;dialog.className="voko-message-dialog";dialog.style.cssText="width:min(440px,calc(100vw - 24px));max-height:calc(100vh - 24px);padding:0;border:0;border-radius:14px;box-shadow:0 22px 70px rgba(15,23,42,.28);overflow:hidden";document.body.appendChild(dialog)}dialog.innerHTML='<div class="home-message-mode-setup" style="display:grid;gap:12px;max-height:calc(100vh - 24px);overflow:auto;padding:20px;color:#344054;font-size:13px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><strong style="font-size:16px">'+title+'</strong><button type="button" style="display:inline-flex;align-items:center;justify-content:center;width:32px;height:32px;margin:0;padding:0;border:0;border-radius:50%;background:transparent;color:#667085;font-size:24px" data-role="close-provider-setup" aria-label="'+I.message_mode_close+'">×</button></div><p style="margin:0;color:#667085;line-height:1.55">'+help+'</p>'+actions.map(function(action){return '<button type="button" class="btn-sm'+(action.secondary?' btn-outline':'')+'" style="margin:0" data-provider-action="'+action.action+'">'+action.label+'</button>'}).join('')+'<button type="button" class="btn-sm btn-outline" style="margin:0" data-role="recheck-provider">'+I.message_mode_recheck+'</button><p data-role="provider-setup-status" style="display:none;margin:0"></p></div>';dialog._messageModeDetails=details;details.open=false;dialog.showModal()}
function renderWorkBuddySetup(details){providerSetupDialog(details,'dlg-workbuddy-setup',I.message_mode_setup_title,I.message_mode_setup_help,[{action:'install_workbuddy',label:I.message_mode_install_action},{action:'login_workbuddy',label:I.message_mode_login_action,secondary:true}])}
function renderQwenSetup(details){providerSetupDialog(details,'dlg-qwen-setup',I.qwen_setup_title,I.qwen_setup_help,[{action:'login_qwen_office',label:I.message_mode_login_action}])}
function renderDuMateSetup(details){providerSetupDialog(details,'dlg-dumate-setup',I.dumate_setup_title,I.message_mode_dumate_notice,[{action:'open_dumate',label:I.message_mode_open_action}])}
