...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/agents/:agentId/external/:principalViewId',async(req,res)=>{
    ...
    try{
      ...
      const selectedContext=contexts.find(item=>item.id===String(req.query.contextId||''))||contexts[contexts.length-1],details=[];
      ...
      const tabs=contexts.length>1?'<style>'+CONVERSATION_TAB_CSS+'</style><div class="conversation-tab-shell"><button type="button" class="conversation-tab-arrow" id="external-tabs-prev" aria-label="'+esc(T('web.payments.prev_page'))+'">‹</button><div class="conversation-tab-rail" id="external-context-tab-rail" role="tablist">'+contexts.map((context,index)=>'<a class="conversation-tab-card'+(context.id===selectedContext.id?' active':'')+'" role="tab" aria-selected="'+(context.id===selectedContext.id)+'" href="/agents/'+encodeURIComponent(agentId)+'/external/'+encodeURIComponent(principalViewId)+'?contextId='+encodeURIComponent(context.id)+'">'+esc(T('web.agent.external.conversation',{index:index+1}))+'</a>').join('')+'</div><button type="button" class="conversation-tab-arrow" id="external-tabs-next" aria-label="'+esc(T('web.payments.next_page'))+'">›</button></div>':'';
      ...
      const messages=details.map((task,index)=>'<section class="card a2a-task-card">'+renderA2ATaskConversation(task,systemName,T,{showTaskMeta:true,taskIdPlain:true,taskLabel:T('web.agent.external.request',{index:index+1}),stateLabel:T('web.agent.external.processing_status'),deliveryLabel:T('web.agent.external.webhook_status'),deliveryValue:webhookState(task)})+'</section>').join('');
      const body=tabs+messages;
      const tabScript=contexts.length>1?'<script>(function(){var rail=document.getElementById("external-context-tab-rail"),prev=document.getElementById("external-tabs-prev"),next=document.getElementById("external-tabs-next");function state(){var max=Math.max(0,rail.scrollWidth-rail.clientWidth);prev.disabled=rail.scrollLeft<2;next.disabled=rail.scrollLeft>=max-2}prev.onclick=function(){rail.scrollBy({left:-Math.max(180,rail.clientWidth*.7),behavior:"smooth"})};next.onclick=function(){rail.scrollBy({left:Math.max(180,rail.clientWidth*.7),behavior:"smooth"})};rail.addEventListener("scroll",state,{passive:true});var active=rail.querySelector(".active");if(active)active.scrollIntoView({inline:"center",block:"nearest"});state()})();</script>':'';
      res.send(renderPage(req,T('web.agent.external.conversation_title',{name:systemName}),body,{showTitle:false,nav:agentNav(agentId,agent?.agentName||agentId,T)+' › '+esc(systemName),footer:renderFooter(T,locale)+tabScript}));
