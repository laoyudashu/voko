...
function agentWsScript(t) {
  const i18nObj = {
    ...
    link_public_tip:t('web.home.short_link.security_tip'),link_private_tip:t('web.home.short_link.private_tip'),save_link:t('common.btn.save'),gen_creating: t('common.home.gen_creating'),
...
function updateVisitorLinkWarning(){var sd=document.getElementById("dlg-short-link-security");if(!sd)return;var selected=sd.querySelector('input[name="link-access"]:checked');sd.querySelector('[data-role="visitor-link-warning-text"]').textContent=selected&&selected.value==='private'?I.link_private_tip:I.link_public_tip}
document.addEventListener("change",function(event){if(event.target.matches('#dlg-short-link-security input[name="link-access"]'))updateVisitorLinkWarning()});
function generateShortLink(t){var aid3=t.dataset.agent,original=t.textContent;t.disabled=true;t.textContent=I.gen_creating;fetch("/api/short-link/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({agentId:aid3,accessMode:document.querySelector('#dlg-short-link-security input[name="link-access"]:checked').value})}).then(function(r){return r.json()}).then(function(d){if(d.success){location.reload()}else{t.disabled=false;t.textContent=original;var m=document.getElementById("toast-msg");if(m)m.textContent=I.gen_failed;var d2=document.getElementById("dlg-toast");if(d2)d2.showModal()}}).catch(function(){t.disabled=false;t.textContent=original})}
...
(function(){
  ...
  function connectWS() {
    try {
      ...
      ws.onclose = function(event) { if (event.code !== 4001) setTimeout(connectWS, 3000); };
...
document.addEventListener("click",function(e){var t=e.target.closest?e.target.closest("[data-role]")||e.target:e.target;
 ...
 else if(t.matches("[data-role=gen-link]")){pendingShortLinkButton=t;var sd=document.getElementById("dlg-short-link-security");if(sd){sd.querySelector('input[value="'+(t.dataset.existing?t.dataset.accessMode:"public")+'"]').checked=true;sd.querySelector('[data-role="confirm-gen-link"]').textContent=t.dataset.existing?I.save_link:I.gen;updateVisitorLinkWarning();sd.showModal()}}
...
function createWebRouter(handlers, db, opts={}){
  ...
  R.get('/',async(req,res,next)=>{
    try{
      ...
      for(const a of pageAgents){
        ...
        if(db){try{accessRow=db.prepare('SELECT short_link_url, imUid FROM agents WHERE agent_id=?').get(a.agentId);if(accessRow&&accessRow.short_link_url){var su=esc(accessRow.short_link_url);visitorValue='<button type="button" class="home-access-value" data-role="gen-link" data-agent="'+esc(a.agentId)+'" data-existing="true" data-access-mode="'+(a.accessMode==='private'?'private':'public')+'" style="border:0;background:none;padding:0;text-align:left;cursor:pointer">'+su.substring(0,35)+(su.length>35?'…':'')+'</button>';visitorAction=copyButton({esc,label:L('web.home.access.copy_visitor'),attrs:'data-voko-copy-value="'+su+'"'})}}catch(ex){}}
        ...
        const qrLabel=L('web.agent.qr.title');
        const qrIcon='<svg width="16" height="16" viewBox="-3 -3 30 30" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M3 3h6v6H3zM15 3h6v6h-6zM3 15h6v6H3zM15 15h3v3h3v3h-6zM12 3v9H3M21 12h-6v3M12 15v6"/></svg>';
        const qrAttrs=' class="voko-copy-button" data-role="visitor-qr" title="'+qrLabel+'" aria-label="'+qrLabel+'"';
        const qrButton=accessRow?.short_link_url&&!auditBlocked?'<a href="/agents/'+encodeURIComponent(a.agentId)+'/visitor-qr"'+qrAttrs+'>'+qrIcon+'</a>':'<button type="button"'+qrAttrs+' disabled>'+qrIcon+'</button>';
  ...
  R.post('/api/agents/:agentId/provider-security/refresh',requireSensitiveLocalAuth,requireSensitiveCsrf,async(req,res)=>{
    try{
      if(typeof opts.dispatcher?.refreshProviderSecurityCapability!=='function')return res.status(503).json({success:false,error:'PROVIDER_CAPABILITY_REFRESH_UNAVAILABLE'});
      const data=await opts.dispatcher.refreshProviderSecurityCapability(String(req.params.agentId),String(req.body?.transportId||'')||undefined);
      res.json({success:true,data});
    }catch(error){res.status(400).json({success:false,error:String(error.message||error)})}
  });
  R.post('/api/agents/:agentId/provider-security/preview',requireSensitiveLocalAuth,requireSensitiveCsrf,(req,res)=>{
    try{
      if(typeof opts.dispatcher?.describeProviderSecurityInvocation!=='function')return res.status(503).json({success:false,error:'PROVIDER_SECURITY_PREVIEW_UNAVAILABLE'});
      const data=opts.dispatcher.describeProviderSecurityInvocation(String(req.params.agentId),String(req.body?.transportId||''),req.body?.config||{});
      res.json({success:true,data});
    }catch(error){res.status(400).json({success:false,error:String(error.message||error)})}
  });
  ...
  R.post('/api/agents/:agentId/provider-security/commit',requireSensitiveLocalAuth,requireSensitiveCsrf,async(req,res)=>{
    try{
      const service=opts.dispatcher?.providerSecurity;
      if(!service)return res.status(503).json({success:false,error:'PROVIDER_SECURITY_UNAVAILABLE'});
      const data=await service.commitAsync(String(req.params.agentId),req.body?.preflightToken,req.body?.confirmation);
  ...
  R.get('/agents/:agentId/security',requireSensitiveLocalAuth,async(req,res,next)=>{
    try{
      ...
      const requestedTransport=String(req.query.transportId||'');
      const data=opts.dispatcher?.inspectProviderSecurity?.(agentId,requestedTransport||undefined)||service.inspect(agentId,requestedTransport||undefined),zh=String(req.locale||'zh').startsWith('zh');
      ...
      const applyAtLabels={next_turn:'下一轮消息生效',session_restart:'会话重建后生效',runtime_start:'运行时启动时生效'};
      const revocationLabels={next_invocation:'下一次调用撤销',restart_runtime:'重启运行时撤销'};
      const visibleControls=data.controls.filter(item=>item.editable&&item.enforcement!=='unsupported');
      const instanceControls=(data.instancePolicy?.controls||[]).filter(item=>item.editable&&item.enforcement!=='unsupported');
      const hasVisibleControls=data.supported&&(visibleControls.length>0||instanceControls.length>0);
      const promptControl=visibleControls.find(item=>item.id==='additionalPrompt');
      const initialPreview=data.transportId&&typeof opts.dispatcher?.describeProviderSecurityInvocation==='function'
        ? opts.dispatcher.describeProviderSecurityInvocation(agentId,data.transportId,data.config||{}):[];
      const previewColor=item=>!item.changed?'#30333a':item.risk==='high'?'#b42318':item.risk==='medium'?'#a85b00':'#1769aa';
      const previewBackground=item=>!item.changed?'transparent':item.risk==='high'?'#fdecec':item.risk==='medium'?'#fff4df':'#e8f1fb';
      ...
      const controlRows=(controls,config,scopeName)=>controls.filter(item=>item.id!=='additionalPrompt').map(item=>{
        ...
        else if(item.editable){const selected=item.values.find(option=>config[item.id]===option.value)||item.values[0],risk=selected&&selected.risk||'low';const riskText=risk==='high'?(zh?'高风险':'High risk'):risk==='medium'?(zh?'中等风险':'Medium risk'):(zh?'低风险':'Low risk');const riskClass=risk==='high'?'badge-offline':risk==='medium'?'badge-pending':'badge-online';field='<div><select name="'+esc(item.id)+'" data-control="'+esc(item.id)+'" data-scope="'+scopeName+'" style="margin:0;max-width:240px">'+item.values.map(option=>'<option value="'+esc(option.value)+'" data-risk="'+esc(option.risk||'low')+'"'+(config[item.id]===option.value?' selected':'')+'>'+esc(option.label)+'</option>').join('')+'</select><div style="margin-top:7px"><span class="badge '+riskClass+'" data-risk-indicator>'+esc(riskText)+'</span></div></div>'}
      ...
      const instanceRows=instanceControls.length?'<section data-policy-scope="agent"><h2>'+(zh?'Agent 公共权限':'Agent-wide permissions')+'</h2><p class="meta">'+(zh?'这些设置由同一 Agent 的多个通信模式共享；保存后会使相关原生会话失效。':'These settings are shared by this Agent across transports; saving invalidates affected native sessions.')+'</p>'+controlRows(instanceControls,data.instancePolicy?.config||{},'agent')+'</section>':'';
      const rows=visibleControls.some(item=>item.id!=='additionalPrompt')?'<section data-policy-scope="transport"><h2>'+(zh?'当前通信模式权限':'Current transport permissions')+'</h2>'+controlRows(visibleControls,data.config||{},'transport')+'</section>':'';
      ...
      const hermesPreview='<div class="card" style="padding:20px 22px"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px"><h2 style="margin-bottom:4px">'+(zh?'权限与安全':'Permissions & security')+'</h2><span id="provider-security-unsaved" class="badge badge-pending" hidden>'+(zh?'有未保存的修改':'Unsaved changes')+'</span></div><p class="meta" style="margin:0 0 18px">'+(zh?'安全提示语用于模型侧纵深防御；安全参数来自当前运行时能力，只有标明强制执行的项目才构成权限边界。':'The prompt is defense in depth. Parameters come from current runtime evidence; only enforced items form a permission boundary.')+'</p>'+(promptControl?'<label for="provider-prompt-editor" style="display:block;color:#1a1a2e;font-size:18px;font-weight:600;margin:0 0 4px">'+(zh?'安全提示语':'Security prompt')+'</label><p class="meta" style="margin:4px 0 8px">'+(zh?'这里编辑的内容会自动追加到每条访客消息，使得Agent意识到这是一个访客信息。':'This content is automatically appended to every visitor message so the Agent recognizes it as visitor information.')+'</p><textarea id="provider-prompt-editor" name="additionalPrompt" data-control="additionalPrompt" data-scope="transport" maxlength="'+Number(promptControl.maxLength||2000)+'" rows="5" style="margin:0;max-width:none;min-height:112px;line-height:1.55;resize:vertical">'+esc(data.config.additionalPrompt||'')+'</textarea><div style="margin:7px 0 18px;text-align:right"><span class="meta" data-char-count></span></div>':'')+'<h3 style="margin:0 0 8px;color:#1a1a2e;font-size:18px;font-weight:600">'+(zh?'安全参数':'Security parameters')+'</h3><div id="provider-command-preview" aria-live="polite" style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:pre-wrap;word-break:break-word;background:#f7f7f8;color:#30333a;border:1px solid #e1e4e8;padding:12px 14px;border-radius:8px;line-height:1.7">'+initialPreviewHtml+'</div><p class="meta" style="margin:8px 0 0">'+(zh?'只有相对已保存策略发生变化的部分会显示彩色高亮；删除的参数会以删除线标出。':'Only segments changed from the saved policy are highlighted; removed parameters are struck through.')+'</p></div>';
      ...
      const observedState=String(data.capabilityEvidence?.observed?.evidenceState||'unknown');
      const selectedTransport=(data.transports||[]).find(item=>item.transportId===data.transportId);
      const agentPolicyBlocked=['applying','drifted','failed','read_only_shared'].includes(String(data.instancePolicy?.nativePolicyState||''));
      const transportNeedsCapability=visibleControls.some(item=>item.id!=='additionalPrompt'&&item.enforcement==='provider_enforced');
      const capabilityBlocked=transportNeedsCapability&&(['changed_unverified','failed'].includes(observedState)||data.capabilityProbing===true);
      const saveBlocked=capabilityBlocked||selectedTransport?.securitySelectable===false||agentPolicyBlocked;
      const blockedNotice=saveBlocked?'<p class="error">'+(selectedTransport?.status==='delivery_stalled'?(zh?'该通信模式存在提交停滞，修复并重新验证前不能保存权限。':'This transport has a stalled submission and cannot save permissions until reverified.'):(zh?'当前运行环境正在检测或已经变化，请重新检测后再保存。':'The runtime is probing or changed. Detect it again before saving.'))+'</p>':'';
      const fixedItems=[...(data.instancePolicy?.controls||[]).filter(item=>!item.editable),...(data.fixedBoundaries||[])];
      const fixed=fixedItems.length?'<section><h2>'+(zh?'固定安全边界与公共证据':'Fixed boundaries and shared evidence')+'</h2>'+fixedItems.map(item=>'<div class="card"><div style="display:flex;justify-content:space-between;gap:12px"><strong>'+esc(item.label)+'</strong><span class="badge '+(item.enforcement==='unsupported'?'badge-offline':'badge-online')+'">'+esc(item.enforcement==='unsupported'?(item.statusLabel||'未受控'):(item.statusLabel||'固定执行'))+'</span></div><p class="meta">'+esc(item.description)+'</p><div class="meta">'+esc(term(item.enforcement,enforcementLabels))+' · '+esc(term(item.applyAt,applyAtLabels))+'</div></div>').join('')+'</section>':'';
      const inactive=Object.keys(data.inactiveConfig||{}).length?'<details class="card"><summary><strong>'+(zh?'当前版本暂未生效':'Not active in this runtime')+'</strong></summary><p class="meta">'+(zh?'这些已保存设置不受当前能力快照支持，VOKO 不会把它们用于本轮参数。':'These saved settings are unsupported by the current capability snapshot and are not used for this invocation.')+'</p><code>'+esc(JSON.stringify(data.inactiveConfig))+'</code></details>':'';
      const form=hasVisibleControls?'<form id="provider-security-form" data-transport="'+esc(data.transportId)+'" data-agent-name="'+esc(data.agentName)+'">'+hermesPreview+instanceRows+rows+fixed+inactive+blockedNotice+confirmation+'<button type="submit"'+(saveBlocked?' disabled':'')+'>'+(zh?'保存设置':'Save settings')+'</button><p id="provider-security-result" class="meta" hidden></p></form>':'<div class="card"><h2>'+esc(unavailableTitle)+'</h2><p><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</p><p class="meta">'+esc(unavailable)+'</p>'+fixed+inactive+'</div>';
      const labels={saving:zh?'正在安全预检…':'Running security preflight…',confirm:zh?'此次修改会扩大权限。请输入智能体名称以确认：':'This change expands authority. Type the Agent name to confirm:',confirmationMismatch:zh?'智能体名称输入错误，请输入页面顶部显示的完整名称。':'The Agent name is incorrect. Enter the full name shown at the top of the page.',saved:zh?'策略已保存，后续尚未提交的消息将使用新权限。':'Policy saved. Unsubmitted messages will use it.',failed:zh?'保存失败':'Save failed'};
      const script='<script>(function(){var form=document.getElementById("provider-security-form");if(!form)return;var out=document.getElementById("provider-security-result"),unsaved=document.getElementById("provider-security-unsaved"),confirmBox=document.getElementById("provider-security-confirmation"),confirmInput=document.getElementById("provider-security-confirmation-input"),L='+jsonForInlineScript(labels)+';function show(text,kind){out.hidden=false;out.textContent=text;out.className=kind||"meta"}function markUnsaved(){if(unsaved)unsaved.hidden=false}function updateRisk(field){var option=field.tagName==="SELECT"?field.options[field.selectedIndex]:null,risk=option&&option.dataset.risk||field.dataset.risk||"low",badge=field.parentElement.querySelector("[data-risk-indicator]");if(badge){badge.textContent=risk==="high"?'+jsonForInlineScript(zh?'高风险':'High risk')+':risk==="medium"?'+jsonForInlineScript(zh?'中等风险':'Medium risk')+':'+jsonForInlineScript(zh?'低风险':'Low risk')+';badge.className="badge "+(risk==="high"?"badge-offline":risk==="medium"?"badge-pending":"badge-online")}var count=field.parentElement.querySelector("[data-char-count]");if(count)count.textContent=field.value.length+" / "+(field.maxLength||2000)}form.querySelectorAll("[data-control]").forEach(function(field){updateRisk(field);field.addEventListener(field.tagName==="TEXTAREA"?"input":"change",function(){updateRisk(field);markUnsaved()})});form.addEventListener("submit",async function(event){event.preventDefault();var button=form.querySelector("button[type=submit]"),transportConfig={},instanceConfig={};form.querySelectorAll("[data-control]").forEach(function(field){(field.dataset.scope==="agent"?instanceConfig:transportConfig)[field.dataset.control]=field.value});button.disabled=true;show(L.saving,"meta");try{var pre=await fetch("/api/agents/"+encodeURIComponent('+jsonForInlineScript(agentId)+')+"/provider-security/preflight",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transportId:form.dataset.transport,config:{transportConfig:transportConfig,instanceConfig:instanceConfig}})}),p=await pre.json();if(!pre.ok||!p.success)throw new Error(p.error||L.failed);var confirmation="";if(p.data.requiresTypedConfirmation){confirmation=confirmInput?confirmInput.value.trim():"";if(!confirmation){if(confirmBox)confirmBox.hidden=false;if(confirmInput)confirmInput.focus();show(L.confirm,"error");return}}var commit=await fetch("/api/agents/"+encodeURIComponent('+jsonForInlineScript(agentId)+')+"/provider-security/commit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({preflightToken:p.data.preflightToken,confirmation:confirmation})}),c=await commit.json();if(!commit.ok||!c.success)throw new Error(c.error||L.failed);if(confirmBox)confirmBox.hidden=true;if(confirmInput)confirmInput.value="";if(unsaved)unsaved.hidden=true;show(L.saved,"success")}catch(error){var message=error.message||L.failed;show(message==="PROVIDER_SECURITY_CONFIRMATION_MISMATCH"?L.confirmationMismatch:message,"error")}finally{button.disabled=false}})})();</script>';
      ...
      const cap=data.capabilityEvidence||{},observed=cap.observed||{},verified=cap.verified||{},state=String(observed.evidenceState||(data.capabilityProbing?'probing':'unknown'));
      const agentNativeVerified=data.instancePolicy?.nativePolicyState==='applied';
      const openclawRuntime=String(data.transportId||'').startsWith('openclaw-');
      const codexRuntime=data.transportId==='codex-cli';
      const stateMeta=openclawRuntime?['badge-pending',zh?'原生权限尚未接入验证':'Native permissions not integrated or verified']:!transportNeedsCapability&&agentNativeVerified?['badge-online',zh?'Agent 原生策略已同步；当前通信模式没有额外权限参数':'Agent native policy is synchronized; this transport has no additional permission flags']:state==='verified'?['badge-online',zh?'当前版本与消息链路已验证':'Current version and delivery verified']:state==='static_compatible'?['badge-pending',zh?'参数静态兼容，尚未完成真实消息验证':'Parameters statically compatible; real delivery not yet verified']:state==='stale_verified'?['badge-pending',zh?'刷新超时，继续使用上次验证参数':'Refresh timed out; using last verified parameters']:state==='probing'?['badge-pending',zh?'正在检测':'Detecting']:state==='changed_unverified'||state==='failed'?['badge-offline',zh?'运行环境尚未验证，自动推送将安全降级':'Runtime unverified; automatic delivery will degrade safely']:['',zh?'尚未检测':'Not detected'];
      const deliveryStatusLabel=item=>item.status==='delivery_stalled'?(zh?'投递停滞':'Delivery stalled'):item.automaticReady?(zh?'投递就绪':'Delivery ready'):item.status==='verification_required'?(zh?'投递待验证':'Delivery verification required'):item.status==='configuration_required'?(zh?'投递待配置':'Delivery configuration required'):item.available===false?(zh?'投递不可用':'Delivery unavailable'):(zh?'投递未就绪':'Delivery not ready');
      const evidenceStatusLabel=item=>item.transportId===data.transportId&&!transportNeedsCapability?(zh?'无额外权限参数':'No extra permission flags'):item.evidenceState==='verified'?(zh?'权限已验证':'Security verified'):item.evidenceState==='static_compatible'?(zh?'参数静态兼容':'Statically compatible'):item.evidenceState==='stale_verified'?(zh?'沿用已验证参数':'Using verified parameters'):item.evidenceState==='changed_unverified'?(zh?'运行时变化待验证':'Changed runtime unverified'):item.evidenceState==='failed'?(zh?'权限检测失败':'Security probe failed'):(zh?'权限未验证':'Security unverified');
      const transportOptions=(data.transports||[]).filter(item=>item.configured).map(item=>'<option value="'+esc(item.transportId)+'"'+(item.transportId===data.transportId?' selected':'')+'>'+esc((modeLabels[item.mode]||item.mode)+' · '+item.transportId+' · '+deliveryStatusLabel(item)+' · '+evidenceStatusLabel(item))+'</option>').join('');
      const transportPicker=transportOptions?'<label style="display:inline-flex;align-items:center;gap:8px;margin:0;white-space:nowrap">'+(zh?'权限对应的通信模式':'Permission transport')+' <select id="provider-security-transport" style="display:inline-block;width:auto;margin:0">'+transportOptions+'</select></label>':'';
      const securityTransport=data.transportId?'<span><strong>'+(zh?'安全适配器':'Security adapter')+':</strong> <code>'+esc(data.transportId)+'</code></span>':'';
      const headerStyle='<style>.provider-security-header{display:grid;grid-template-columns:max-content max-content max-content minmax(260px,1fr);align-items:center;gap:8px 18px}.provider-security-header>span,.provider-security-header>label{min-width:0}.provider-security-header select{max-width:100%}@media(max-width:900px){.provider-security-header{grid-template-columns:max-content minmax(260px,1fr)}.provider-security-header>label{grid-column:1/-1}}@media(max-width:600px){.provider-security-header{grid-template-columns:1fr}.provider-security-header>label{grid-column:auto}}</style>';
      const supportedHeader=headerStyle+'<div class="info-bar provider-security-header"><span><strong>'+(zh?'智能体框架':'Agent framework')+':</strong> '+esc(data.backendType||'-')+'</span><span><strong>'+(zh?'消息推送模式':'Message delivery mode')+':</strong> '+esc(modeLabels[deliveryMode]||deliveryMode)+'</span>'+securityTransport+transportPicker+'</div>';
      const callLabels={documented_contract:zh?'已核对版本参数；真实投递未验收':'Version parameters reviewed; delivery not tested',parameters_checked:zh?'参数检查通过；版本未验收':'Parameters checked; version not validated',protocol_compatible:zh?'协议兼容；真实投递未验收':'Protocol compatible; delivery not tested',unverified:zh?'调用兼容性待验证':'Call compatibility unverified'};
      const openclawDetails=openclawRuntime?'<p>'+esc(callLabels[observed.callCompatibility]||callLabels.unverified)+'</p><p>'+esc(zh?'当前仅支持补充提示语，不构成 Shell、文件、浏览器或网络权限限制。原生权限尚未接入验证。':'Only additional instructions are available. They do not restrict shell, files, browser or network access. Native permissions are not integrated or verified.')+'</p><div class="meta">'+esc(zh?'版本来源：':'Version source: ')+esc(data.transportId==='openclaw-ws'?(zh?'已认证 Gateway':'Authenticated Gateway'):(zh?'本地 CLI 包':'Local CLI package'))+' · '+esc(zh?'协议：':'Protocol: ')+esc(observed.protocolVersion||'-')+' · VOKO Node: '+esc(observed.nodeVersion||'-')+'</div>':'';
      const codexDetails=codexRuntime?'<div class="card" id="codex-compatibility"><p>'+esc(observed.securityVerification==='CODEX_SANDBOX_CANARY_VERIFIED'?(zh?'CLI 参数与本机沙箱检查通过；模型回复和消息投递需另行验证。':'CLI parameters and local sandbox checks passed; model replies and message delivery require separate verification.'):(zh?'当前运行文件的沙箱尚未验证。请重新检测；检测失败时不会开放沙箱权限选项。':'The current executable sandbox is unverified. Detect again; failed checks do not enable sandbox controls.'))+'</p><p class="meta">'+esc(zh?'只读仍允许执行命令和广泛读取宿主机文件。网络没有独立配置开关。':'Read-only still permits commands and broad host file reads. Network has no independent configuration switch.')+'</p><code>'+esc(observed.securityVerification||'CODEX_RUNTIME_NOT_PROBED')+'</code>'+(observed.securityDiagnostic?.message?'<p class="meta">'+esc(observed.securityDiagnostic.message)+'</p>':'')+'</div>':'';
      const environment='<div class="card" id="provider-runtime-card"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap"><div><h2 style="margin:0 0 8px">'+(zh?'当前运行环境':'Current runtime')+'</h2><span class="badge '+stateMeta[0]+'">'+esc(stateMeta[1])+'</span></div>'+(data.transportId?'<button type="button" id="provider-runtime-refresh" class="btn-sm btn-outline" style="margin:0">'+(zh?'重新检测':'Detect again')+'</button>':'')+'</div><div class="meta" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px 18px;margin-top:14px"><span><strong>'+(zh?'框架版本':'Framework version')+':</strong> '+esc(observed.frameworkVersion||(!openclawRuntime&&!codexRuntime&&verified.frameworkVersion)||'-')+'</span><span><strong>'+(zh?'运行时版本':'Runtime version')+':</strong> '+esc(observed.runtimeVersion||(!openclawRuntime&&!codexRuntime&&verified.runtimeVersion)||'-')+'</span><span><strong>'+(zh?'系统环境':'System')+':</strong> '+esc((observed.platform||process.platform)+' / '+(observed.arch||process.arch))+'</span><span><strong>'+(zh?'最近检测':'Last detected')+':</strong> '+esc(observed.observedAt?new Date(observed.observedAt).toLocaleString(req.locale||'zh-CN'):'-')+'</span></div></div>';
      const runtimeScript='<script>(function(){var aid='+jsonForInlineScript(agentId)+',form=document.getElementById("provider-security-form"),button=document.getElementById("provider-runtime-refresh"),picker=document.getElementById("provider-security-transport"),command=document.getElementById("provider-command-preview"),timer=null;function config(){var value={instanceConfig:{},transportConfig:{}};if(form)form.querySelectorAll("[data-control]").forEach(function(field){(field.dataset.scope==="agent"?value.instanceConfig:value.transportConfig)[field.dataset.control]=field.value});return value}function render(items){if(!command||!Array.isArray(items))return;command.textContent="";items.forEach(function(item,index){if(index)command.appendChild(document.createTextNode(" "));var span=document.createElement("span"),removed=item.change==="removed";span.dataset.change=item.change||"unchanged";span.textContent=(removed?'+jsonForInlineScript(zh?'移除 ':'Remove ')+':"")+item.text;span.style.display="inline-block";span.style.padding="1px 5px";span.style.borderRadius="5px";span.style.fontWeight=item.changed?"600":"400";if(removed){span.style.textDecoration="line-through";span.style.opacity=".72"}var risk=item.risk||"low";span.style.color=item.changed?(risk==="high"?"#b42318":risk==="medium"?"#a85b00":"#1769aa"):"#30333a";span.style.background=item.changed?(risk==="high"?"#fdecec":risk==="medium"?"#fff4df":"#e8f1fb"):"transparent";command.appendChild(span)})}function preview(){if(!form||!command)return;fetch("/api/agents/"+encodeURIComponent(aid)+"/provider-security/preview",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transportId:form.dataset.transport,config:config()})}).then(function(r){return r.json()}).then(function(x){if(x.success)render(x.data)}).catch(function(){})}if(picker)picker.addEventListener("change",function(){var url=new URL(location.href);url.searchParams.set("transportId",picker.value);location.href=url.toString()});if(form){form.addEventListener("input",function(){clearTimeout(timer);timer=setTimeout(preview,120)});form.addEventListener("change",preview);preview()}if(button)button.addEventListener("click",function(){button.disabled=true;button.textContent='+(jsonForInlineScript(zh?'检测中…':'Detecting…'))+';fetch("/api/agents/"+encodeURIComponent(aid)+"/provider-security/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transportId:form?form.dataset.transport:""})}).then(function(r){return r.json()}).then(function(x){if(!x.success)throw new Error(x.error||"refresh failed");location.reload()}).catch(function(error){button.disabled=false;button.textContent=error.message})})})();</script>';
      const capabilityPollScript='<script>(function(){var aid='+jsonForInlineScript(agentId)+',form=document.getElementById("provider-security-form"),transport=form&&form.dataset.transport||"",agentKey="voko-security-agent-draft:"+aid,transportKey="voko-security-transport-draft:"+aid+":"+transport;function values(scope){var out={};if(form)form.querySelectorAll("[data-control][data-scope=\""+scope+"\"]").forEach(function(field){out[field.dataset.control]=field.value});return out}function restore(key,scope){var saved=sessionStorage.getItem(key);if(!form||!saved)return;try{var draft=JSON.parse(saved);form.querySelectorAll("[data-control][data-scope=\""+scope+"\"]").forEach(function(field){if(Object.prototype.hasOwnProperty.call(draft,field.dataset.control))field.value=draft[field.dataset.control]});var unsaved=document.getElementById("provider-security-unsaved");if(unsaved)unsaved.hidden=false}catch(_){sessionStorage.removeItem(key)}}restore(agentKey,"agent");restore(transportKey,"transport");if(form){form.dispatchEvent(new Event("change",{bubbles:true}));form.addEventListener("input",function(){sessionStorage.setItem(agentKey,JSON.stringify(values("agent")));sessionStorage.setItem(transportKey,JSON.stringify(values("transport")))});form.addEventListener("submit",function(){sessionStorage.removeItem(agentKey);sessionStorage.removeItem(transportKey)})}'+(data.capabilityProbing?'var attempts=0,poll=setInterval(function(){attempts+=1;fetch("/api/agents/"+encodeURIComponent(aid)+"/provider-security",{headers:{Accept:"application/json"}}).then(function(r){return r.json()}).then(function(x){if(x.success&&!x.data.capabilityProbing){clearInterval(poll);location.reload()}else if(attempts>=30)clearInterval(poll)}).catch(function(){if(attempts>=30)clearInterval(poll)})},1000);':'')+'})();</script>';
      const body=supportedHeader+environment+openclawDetails+codexDetails+(hasVisibleControls?scope:'')+form+'<p><a href="/agents/'+esc(agentId)+'">← '+(zh?'返回智能体':'Back to Agent')+'</a></p>';
      res.send(renderPage(req,title,body,{nav:agentNav(agentId,agent.agentName||agentId,req.t)+' › '+esc(title),footer:renderFooter(req.t,req.locale)+script+runtimeScript+capabilityPollScript}));
    }catch(error){next(error)}
  });
  ...
  R.get('/agents/:agentId',async(req,res,next)=>{
    try{
      ...
      if(convPages>1){
       ...
       const securityLabel=L('web.agent.op.security');
       const aclOps='<h2>'+L('web.agent.acl_title')+'</h2><div class="ops agent-detail-ops" style="grid-template-columns:repeat(auto-fit,minmax(min(100%,160px),1fr))"><a href="/agents/'+aId+'/security" class="op-card" data-agent-kind="link" data-agent="nav_card">'+securityLabel+'</a><a href="/agents/'+aId+'/caps" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('common.btn.caps')+'</a><a href="/capabilities?agentId='+aId+'" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.home.op.capabilities')+'</a><a href="/send-message?agentId='+aId+'" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.home.op.send_message')+'</a><a href="/interventions?agentId='+aId+'" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.op.interventions')+'</a><a href="/agents/'+aId+'/invite" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.invite.title')+'</a><a href="/agents/'+aId+'/payment-auth" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.home.op.payments')+'</a><a href="/agents/'+aId+'/visibility" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.op.visibility')+'</a><a href="/agents/'+aId+'/whitelist" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.op.allowlist')+'</a><a href="/agents/'+aId+'/blacklist" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.op.blocklist')+'</a><a href="/agents/'+aId+'/pricing" class="op-card" data-agent-kind="link" data-agent="nav_card">'+L('web.agent.op.pricing')+'</a></div>';;;
      // 群列表（群 Tab）
...
try{const r=await handlers.list_access_lists({agentId,listType:'whitelist',limit,offset,keyword});const es=r.data||r.entries||r.accessList||[],total=r.total||es.length;totalPages=Math.ceil(total/limit);let wlNickMap={};if(es.length){try{const vids=es.map(e=>e.visitor_id||e.visitorId||'').filter(Boolean);if(vids.length){const rows=db.prepare('SELECT uid, nickname FROM user_cache WHERE uid IN ('+vids.map(()=>'?').join(',')+')').all(...vids);rows.forEach(r=>{wlNickMap[r.uid]=r.nickname||'';});}}catch(_){}const wlName=(vid)=>{const n=wlNickMap[vid];return n?esc(n)+' ('+esc(vid)+')':esc(vid);};const wlSource=(e)=>e.source==='same_owner_default'?(e.auto_trust_disabled?'已关闭自动信任':'同一主人自动信任'):(e.source==='outbound_contact'?'主动联系自动加入':'手动添加');listHtml='<div class="table-wrap"><table><thead><tr><th>'+L('web.agent.whitelist.col.visitor')+'</th><th>来源</th><th>'+L('web.agent.whitelist.col.reason')+'</th><th style="text-align:center">'+L('web.agent.whitelist.col.action')+'</th></tr></thead><tbody>'+es.map(e=>'<tr><td>'+wlName(e.visitor_id||e.visitorId||'')+'</td><td>'+esc(wlSource(e))+'</td><td>'+esc(e.reason||'-')+'</td><td style="text-align:center"><form method="POST" action="/agents/'+esc(agentId)+'" data-voko-access-list style="display:inline"><input type="hidden" name="_action" value="remove_whitelist"><input type="hidden" name="visitorId" value="'+esc(e.visitor_id||e.visitorId||'')+'"><button type="submit" class="btn-xs btn-outline" style="margin:0;padding:2px 8px;font-size:11px;min-height:auto">'+L('common.btn.remove')+'</button></form></td></tr>').join('\n')+'</tbody></table></div>'}}catch{}
  ...
  R.get('/agents/:agentId/visitor-qr',async(req,res,next)=>{
    try{
      ...
      const row=db?.prepare('SELECT short_link_url, icon_url FROM agents WHERE agent_id=?').get(agentId);
      const link=String(row?.short_link_url||'');
      let valid=false;
      try{const url=new URL(link);valid=['https:','http:'].includes(url.protocol)&&!url.username&&!url.password}catch{}
    ...
    try{
      ...
      res.send(renderAgentFormPage(T('web.agent.access.title'),agentId,agent.agentName||agentId,
        '<p>'+T('web.agent.access.current')+'：<strong>'+(priv?T('web.agent.access.private_desc'):T('web.agent.access.public_desc'))+'</strong></p>'+actionForm(agentId,'set_access_mode',[
          {id:'am',name:'enabled',label:T('web.agent.op.access'),type:'select',val:String(priv),options:{false:T('web.agent.access.public_opt')+(!priv?' ('+T('web.agent.access.current')+')':''),true:T('web.agent.access.private_opt')+(priv?' ('+T('web.agent.access.current')+')':'')}},
        ],T('common.btn.save'),null,'access.mode.set'),req.t,req.locale))
    }catch(e){next(e)}
  });
  ...
  const verifyA2ACard=async(req,res)=>{
  ...
  R.get('/agents/:agentId/caps/verify-a2a-card',verifyA2ACard);
  R.post('/agents/:agentId/caps/verify-a2a-card',verifyA2ACard);
...
+'- Operation results: ActionStatusType JSON-LD\n');});
  ...
  R.post('/api/short-link/create', async (req, res) => {
    try {
      const { agentId, accessMode = 'public' } = req.body;
      if (!agentId) return res.json({ success: false, error: '缺少 agentId' });
      if (!['public', 'private'].includes(accessMode)) return res.status(400).json({ success: false, error: 'Invalid access mode' });
      let imUid = '';
      let ownerEmail = '';
      if (db) {
        try {
      ...
      const modeResult = await handlers.set_private_mode({ agentId, enabled: accessMode === 'private' });
      if (modeResult.success === false || modeResult.error !== undefined) return res.json({ success: false, error: modeResult.error || 'Unable to save access mode' });
      const existing = db.prepare('SELECT short_link_url FROM agents WHERE agent_id=?').get(agentId);
      if (existing?.short_link_url) return res.json({ success: true, data: { shortUrl: existing.short_link_url } });
      const title = agentId;
