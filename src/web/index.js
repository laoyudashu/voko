...
footer:'<script>(function(){try{var ws=new WebSocket("ws://"+location.host+"/ws");ws.onmessage=function(e){try{var d=JSON.parse(e.data);if(d.event==="owner-intervention:email-reply"||d.event==="owner-intervention:new")setTimeout(function(){location.reload()},800)}catch(_){}};ws.onclose=function(){setTimeout(arguments.callee,3000)}}catch(_){}})();</script>'}))
  ...
  R.get('/audit-rules',async(req,res,next)=>{
    try{
      ...
      const modelCard='<details class="card"><summary style="cursor:pointer;font-size:1.17em;font-weight:600">'+L('web.audit.model.title')+' <span class="badge '+(modelCfg.enabled?'badge-online':'badge-pending')+'" style="margin-left:6px">'+esc(modelState)+'</span></summary>'
        ...
        +'<label for="safety-model-preset">'+L('web.audit.model.preset')+'</label><select id="safety-model-preset"><option value="custom"'+(selectedPreset?'':' selected')+'>'+L('web.audit.model.custom')+'</option>'+presetOptions+'</select>'
        +'<label>'+L('web.audit.model.api_key')+'</label><input type="password" name="apiKey" value="" placeholder="'+esc(modelCfg.hasApiKey?modelCfg.apiKeyMasked:L('web.audit.model.api_key_empty'))+'" autocomplete="new-password">'
        +(modelCfg.hasApiKey?'<p class="meta" style="margin:3px 0">'+L('web.audit.model.api_key_configured')+' '+esc(modelCfg.apiKeyMasked)+'</p>':'')
        ...
        +'<div class="full"><label>'+L('web.audit.model.base_url')+'</label><input id="safety-base-url" name="baseUrl" value="'+esc(modelCfg.baseUrl||'')+'" placeholder="https://api.example.com"></div></div>'
        +'<details style="margin-top:12px"><summary style="cursor:pointer;font-weight:600">'+L('web.audit.model.advanced')+'</summary><div class="form-grid">'
        ...
        +'</form><script>(function(){var presets='+presetData+',form=document.getElementById("safety-model-form"),result=document.getElementById("safety-model-result"),pick=document.getElementById("safety-model-preset"),type=document.getElementById("safety-api-type"),url=document.getElementById("safety-base-url"),model=document.getElementById("safety-model-id"),medium=document.getElementById("safety-medium"),high=document.getElementById("safety-high"),mediumOut=document.getElementById("safety-medium-output"),highOut=document.getElementById("safety-high-output");pick.addEventListener("change",function(){var p=presets.find(function(x){return x.id===pick.value});if(!p){type.value="openai-chat";url.value="";model.value="";return}type.value=p.apiType;url.value=p.baseUrl;model.value=p.modelId});function show(){mediumOut.value=Math.round(Number(medium.value)*100)+"%";highOut.value=Math.round(Number(high.value)*100)+"%"}medium.addEventListener("input",function(){if(Number(medium.value)>Number(high.value))high.value=medium.value;show()});high.addEventListener("input",function(){if(Number(high.value)<Number(medium.value))medium.value=high.value;show()});form.addEventListener("submit",async function(event){event.preventDefault();var mode=event.submitter&&event.submitter.value==="test"?"test":"save",buttons=form.querySelectorAll("button[type=submit]"),data={};new FormData(form).forEach(function(value,key){if(key!=="mode"&&key!=="_csrf")data[key]=value});data.enabled=form.querySelector("[name=enabled]").checked;buttons.forEach(function(button){button.disabled=true});result.hidden=false;result.className="meta";result.textContent='+JSON.stringify(T('web.audit.model.processing'))+';try{var endpoint=mode==="test"?"/api/audit-rules/model-assistance/test":"/api/audit-rules/model-assistance",response=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify(data)}),payload=await response.json().catch(function(){return{}});if(!response.ok||!payload.success)throw new Error(payload.error||'+JSON.stringify(T('common.action.failed'))+');if(mode==="test")form.querySelector("[name=enabled]").checked=false;result.className="success";result.textContent=mode==="test"?'+JSON.stringify(T('web.audit.model.test_success'))+':'+JSON.stringify(T('web.audit.model.save_success'))+'}catch(error){result.className="error";result.textContent=error.message||'+JSON.stringify(T('common.action.failed'))+'}finally{buttons.forEach(function(button){button.disabled=false})}});show()})();</'+'script></div></details>';
      const body=modelCard+'<form method="GET" action="/audit-rules" style="display:flex;gap:8px;margin-bottom:10px">'
        ...
        +pg
        +'<div class="card audit-rule-add-card"><h3>'+L('web.audit.add_title')+'</h3><form method="POST" action="/audit-rules"><input type="hidden" name="action" value="add"><div class="form-grid audit-rule-form-grid"><div><label for="rd">'+L('web.audit.lbl.direction')+'</label><select id="rd" name="direction">'+dOpts+'</select></div><div><label for="ra">'+L('web.audit.lbl.action')+'</label><select id="ra" name="actionType">'+aOpts+'</select></div><div class="full"><label for="rk">'+L('web.audit.lbl.keyword')+'</label><input type="text" id="rk" name="keyword" required></div><div class="full"><label for="rp">'+L('web.audit.lbl.prompt')+'</label><input type="text" id="rp" name="prompt"></div></div><div style="margin-top:12px"><button type="submit">'+L('common.btn.add')+'</button></div></form></div>';
