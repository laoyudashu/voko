...
function createMcpServer(toolHandlers: ToolHandlerMap, options: McpServerOptions = {}) {
  ...
  server.tool('voko_refresh_delivery_channels', 'Refresh the running Provider delivery readiness for one local Agent.', {
    agentId: z.string().describe(T('mcp.param.agentId')),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.refresh_delivery_channels(params)) }] }));
  server.tool('voko_verify_delivery_channel', 'Run the Provider real loopback check required before enabling an automatic delivery transport.', {
    agentId: z.string().describe(T('mcp.param.agentId')), providerId: z.string().min(1),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.verify_delivery_channel(params)) }] }), { destructiveHint: true });
  server.tool('voko_select_delivery_channel', 'Temporarily select one already configured Provider transport for real-device verification.', {
    agentId: z.string().describe(T('mcp.param.agentId')), mode: z.string().min(1), providerId: z.string().min(1),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.select_delivery_channel(params)) }] }), { destructiveHint: true });
  server.tool('voko_inspect_provider_security', 'Read the effective Provider security policy and cached runtime capability evidence.', {
    agentId: z.string().describe(T('mcp.param.agentId')), transportId: z.string().optional(),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.inspect_provider_security(params)) }] }), { readOnlyHint: true });
  server.tool('voko_inspect_provider_runtime', 'Read the exact Provider transport runtime version, fingerprint, platform, and sandbox evidence.', {
    agentId: z.string().describe(T('mcp.param.agentId')), transportId: z.string().min(1),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.inspect_provider_runtime(params)) }] }), { readOnlyHint: true });
  server.tool('voko_refresh_provider_security_capability', 'Refresh runtime capability evidence for an exact Provider transport.', {
    agentId: z.string().describe(T('mcp.param.agentId')), transportId: z.string().optional(),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.refresh_provider_security_capability(params)) }] }));
  server.tool('voko_preview_provider_security_invocation', 'Preview the redacted Provider invocation generated by the production planner.', {
    agentId: z.string().describe(T('mcp.param.agentId')), transportId: z.string().min(1), config: z.record(z.string(), z.string()).optional(),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.preview_provider_security_invocation(params)) }] }), { readOnlyHint: true });
  server.tool('voko_preflight_provider_security', 'Preflight a Provider security policy change without applying it.', {
    agentId: z.string().describe(T('mcp.param.agentId')), transportId: z.string().min(1), config: z.record(z.string(), z.string()),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.preflight_provider_security(params)) }] }));
  server.tool('voko_commit_provider_security', 'Commit a previously preflighted Provider security policy change.', {
    agentId: z.string().describe(T('mcp.param.agentId')), preflightToken: z.string().min(1), confirmation: z.string().optional(),
  }, async (params: unknown) => ({ content: [{ type: 'text', text: JSON.stringify(await toolHandlers.commit_provider_security(params)) }] }), { destructiveHint: true });
  server.tool('voko_inspect_provider_turn_evidence', 'Read redacted policy lease and native session binding evidence for one completed Provider Turn.', {
