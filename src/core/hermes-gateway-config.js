...
function readHermesGatewayConfig(configPath, { portOnly = false } = {}) {
  ...
  for (const keys of [['platforms'], prefix, [...prefix, 'extra']]) {
    const value = keys.reduce((current, key) => current?.[key], values);
    if (value != null && Object.prototype.toString.call(value) !== '[object Object]') throw new Error('HERMES_CONFIG_INVALID');
  }
  const api = values.platforms?.api_server;
  const apiKey = portOnly ? '' : api?.extra?.key;
  const port = api?.extra?.port;
  const enabled = portOnly ? false : api?.enabled;
  ...
  return { configPath, source, document, values, apiKey: apiKey || '', port: port || null, enabled: enabled === true };
}

/** Only collect API connection candidates; authentication decides which the runtime uses. */
function readHermesGatewayEnvironment(configPath, env = process.env, { portOnly = false } = {}) {
  let source = '';
  ...
  for (const match of source.matchAll(entries)) {
    ...
    if (portOnly && key === 'API_SERVER_KEY') continue;
    const raw = match[3];
  ...
  const apiKey = portOnly ? '' : values.API_SERVER_KEY || '';
  ...
  const validPort = Number.isSafeInteger(port) && port > 0 && port <= 65535;
  if (portOnly && values.API_SERVER_PORT && !validPort) throw new Error('HERMES_ENV_PORT_INVALID: 无法确定其他 profile 的 API 端口');
  return { apiKey, port: validPort ? port : null,
...
function writeHermesGatewayConfig(config, apiKey, port) {
  const { configPath, source, document, values } = config;
  ...
  for (const keys of [['platforms'], ['platforms', 'api_server'], ['platforms', 'api_server', 'extra']]) {
    if (!YAML.isMap(document.getIn(keys, true))) {
      const effective = keys.reduce((current, key) => current?.[key], values);
      document.setIn(keys, document.createNode(effective || {}));
    }
  }
  ...
  const api = values.platforms?.api_server || {};
  const expected = { ...values, platforms: { ...values.platforms, api_server: {
    ...api, enabled: true, extra: { ...api.extra, key: apiKey, port },
  } } };
  if (!isDeepStrictEqual(document.toJS(), new YAML.Document(expected).toJS())) {
    throw new Error('HERMES_CONFIG_WRITE_UNSUPPORTED: 无法安全更新此 YAML 结构，原文件未修改');
  }
