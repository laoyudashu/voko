...
async function setAgentStatus(opts?: SetAgentStatusOptions): Promise<SetAgentStatusResult> {
  ...
  if (!isBinaryFlag(status) || !isBinaryFlag(visibility)) {
    return { success: false, error: t('errors.agent_status.invalid_values') };
  }

  try {
    const row = db.prepare(`SELECT did, private_key FROM agents WHERE agent_id = ?`).get(agentId) as AgentCredentialRow | undefined;
