...
class A2ARegistrationService {
  ...
  async ensureRegistered(): Promise<Record<string, any>> {
    ...
    const agents: Array<{ publicAgentId: string; localAgentId: string; publicEnabled: boolean }> = this.options.mainDb.prepare(`SELECT agent_id FROM agents WHERE publish_status='published' AND LOWER(TRIM(owner_email))=? ORDER BY agent_id`)
    ...
    const fingerprint = crypto.createHash('sha256').update(JSON.stringify({ registrationVersion: 3, agents, keyId: identity.keyId })).digest('hex');
