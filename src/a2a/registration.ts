...
import { A2APublicationStore } from './publication-store';
...
class A2ARegistrationService {
  ...
  async ensureRegistered(): Promise<Record<string, any>> {
    ...
    const publications = new A2APublicationStore(this.options.a2aDb);
    const agents: Array<{ publicAgentId: string; localAgentId: string; publicEnabled: boolean }> = this.options.mainDb.prepare(`SELECT agent_id,did FROM agents WHERE publish_status='published' AND LOWER(TRIM(owner_email))=? ORDER BY agent_id`)
      .all(this.options.ownerEmail.toLowerCase()).map((row: any) => ({ publicAgentId: serverAgentIdFromDid(row.did) || row.agent_id, localAgentId: row.agent_id,
        publicEnabled: publications.isPublicEnabled(row.agent_id) }));
    ...
    const fingerprint = crypto.createHash('sha256').update(JSON.stringify({ registrationVersion: 4, agents, keyId: identity.keyId })).digest('hex');
