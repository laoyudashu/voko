...
import { serverAgentIdFromDid } from '../core/agent-invitations';
...
class A2ARegistrationService {
  ...
  async ensureRegistered(): Promise<Record<string, any>> {
    ...
    const agents: Array<{ publicAgentId: string; localAgentId: string }> = this.options.mainDb.prepare(`SELECT agent_id,did FROM agents WHERE publish_status='published' AND LOWER(TRIM(owner_email))=? ORDER BY agent_id`)
      .all(this.options.ownerEmail.toLowerCase()).map((row: any) => ({ publicAgentId: serverAgentIdFromDid(row.did) || row.agent_id, localAgentId: row.agent_id }));
    ...
    // Bump the registration protocol when the per-Agent A2A allowlist is removed.
    // This forces existing Lite installations to refresh their server bindings
    // instead of reusing a cached pre-full-rollout response.
    const fingerprint = crypto.createHash('sha256').update(JSON.stringify({ registrationVersion: 6, agents, keyId: identity.keyId })).digest('hex');
    ...
    const config = { fingerprint, deviceId, token: result.token, mailboxUrl: `${baseUrl}${result.mailboxPath}`,
      ...
      agentBindings: Array.isArray(result.agentBindings) ? result.agentBindings : agents.map((agent) => ({ ...agent, bindingGeneration: 1 })),
