...
class A2ABridgeWorker {
  ...
  async pollOnce(): Promise<{ claimed: number; processed: number; uncertain: number }> {
    const claim = await this.options.client.claim(); let processed = 0; let uncertain = 0;
    for (const item of claim.items) {
      ...
      const accepted = this.options.store.acceptCommand(envelope.eventId, envelope.gatewayTaskId, envelope.sequence, envelope.operation);
