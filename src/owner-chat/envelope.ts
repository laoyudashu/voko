...
const KINDS = new Set(['message','receipt','reply','event']);
const OPERATIONS = new Set(['message','accepted','working','reply','failed']);

interface OwnerChatEnvelope {
  version: typeof OWNER_CHAT_VERSION; kind: 'message'|'receipt'|'reply'|'event'; messageId: string; clientMessageId: string;
  conversationId: string; ownerIdentityId: string; ownerImUid: string; agentId: string; ownershipEpoch: number;
  conversationEpoch: number; sequence: number; operation: 'message'|'accepted'|'working'|'reply'|'failed'; contentType: 1|2|3;
